...
namespace {
	...
	// Single float/double members, no padding — author-asserted byte hashes for the
	// checksum enforcement.
	ECS_CHECKSUM_BYTES(LifeC)
	struct LifeD {
		float y = 0.0f;
	};
	ECS_CHECKSUM_BYTES(LifeD)
	struct LifeE {
		int64_t z = 0;
	};
	struct LifeF {
	...
	ECS_CHECKSUM_BYTES(LifeG)
	struct LifeH {
		double s = 0.0;
	};
	ECS_CHECKSUM_BYTES(LifeH)
}
