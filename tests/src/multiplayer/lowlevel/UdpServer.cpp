...
inline static std::unique_ptr<UdpServer> create_server(IpAddress const& p_address, int p_port) {
	std::unique_ptr<UdpServer> server = std::make_unique<UdpServer>();
...
inline static std::unique_ptr<UdpClient> create_client(IpAddress const& p_address, int p_port) {
...
inline static std::unique_ptr<UdpClient> accept_connection(UdpServer* p_server) {
	wait_for_condition([&]() {
	...
	std::unique_ptr<UdpClient> client_from_server = p_server->take_next_client_as<UdpClient>();
...
TEST_CASE("UdpServer accept connections and handle data", "[UdpServer][UdpServer-accept-connections]") {
	std::unique_ptr<UdpServer> server = create_server(LOCALHOST, PORT);
	std::unique_ptr<UdpClient> client = create_client(LOCALHOST, PORT);
	...
	std::unique_ptr<UdpClient> client_from_server = accept_connection(server.get());
...
TEST_CASE("UdpServer handle multiple clients", "[UdpServer][UdpServer-multiple-clients]") {
	...
	PacketBuilder pb;
	for (int i = 0; i < 5; i++) {
		std::unique_ptr<UdpClient> c = create_client(LOCALHOST, PORT);
	...
	for (int i = 0; i < clients.size(); i++) {
		std::unique_ptr<UdpClient> cfs = accept_connection(server.get());
	...
	wait_for_condition([&]() {
		...
		for (std::unique_ptr<UdpClient>& c : clients) {
