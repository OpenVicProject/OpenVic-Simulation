...
inline static std::unique_ptr<TcpServer> create_server(IpAddress const& p_address, int p_port) {
	std::unique_ptr<TcpServer> server = std::make_unique<TcpServer>();
...
inline static std::unique_ptr<TcpPacketStream> create_client(IpAddress const& p_address, int p_port) {
	std::unique_ptr<TcpPacketStream> client = std::make_unique<TcpPacketStream>();
...
inline static std::unique_ptr<TcpPacketStream> accept_connection(TcpServer* p_server) {
	wait_for_condition([&]() {
		return p_server->is_connection_available();
	});

	REQUIRE(p_server->is_connection_available());
	std::unique_ptr<TcpPacketStream> client_from_server = p_server->take_packet_stream();
...
TEST_CASE("TcpServer accept connections and handle data", "[TcpServer][TcpServer-accept-connections]") {
	std::unique_ptr<TcpServer> server = create_server(LOCALHOST, PORT);
	std::unique_ptr<TcpPacketStream> client = create_client(LOCALHOST, PORT);
	std::unique_ptr<TcpPacketStream> client_from_server = accept_connection(server.get());
...
TEST_CASE("TcpServer handle multiple clients", "[TcpServer][TcpServer-multiple-clients]") {
	...
	std::vector<std::unique_ptr<TcpPacketStream>> clients;
	for (int i = 0; i < 5; i++) {
		clients.push_back(create_client(LOCALHOST, PORT));
	}

	std::vector<std::unique_ptr<TcpPacketStream>> clients_from_server;
	for (int i = 0; i < 5; i++) {
	...
	wait_for_condition([&]() {
		...
		for (std::unique_ptr<TcpPacketStream>& c : clients) {
...
TEST_CASE("TcpServer refuse new connections after close", "[TcpServer][TcpServer-close-refuse]") {
	...
	std::unique_ptr<TcpPacketStream> new_client = std::make_unique<TcpPacketStream>();
