...
TEST_CASE(
	"IpAddress zpp::bits out then in",
	"[IpAddress][IpAddress-out-in][utility][zpp::bits][zpp::bits-out-in-IpAddress]",
) {
	static constexpr IpAddress ipv6 = { 0xab65002f, 0x002253f3, 0, 0x0321fe00, true };

	std::array<uint8_t, sizeof(ipv6)> buffer; // NOLINT
	zpp::bits::out out(buffer);
	CHECK(out(ipv6) == std::errc {});

	zpp::bits::in in(buffer);

	IpAddress should_ipv6;
	CHECK(in(should_ipv6) == std::errc {});
	CHECK(should_ipv6 == ipv6);
}
