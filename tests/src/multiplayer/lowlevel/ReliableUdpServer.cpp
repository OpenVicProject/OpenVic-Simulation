...
inline static std::unique_ptr<ReliableUdpServer> create_server(IpAddress const& p_address, int p_port) {
	std::unique_ptr<ReliableUdpServer> server = std::make_unique<ReliableUdpServer>();
...
inline static std::unique_ptr<ReliableUdpClient> create_client(IpAddress const& p_address, int p_port) {
	std::unique_ptr<ReliableUdpClient> client = std::make_unique<ReliableUdpClient>();
...
inline static std::unique_ptr<ReliableUdpClient> accept_connection(ReliableUdpServer* p_server) {
	wait_for_condition([&]() {
	...
	std::unique_ptr<ReliableUdpClient> client_from_server = p_server->take_next_client_as<ReliableUdpClient>();
...
TEST_CASE("ReliableUdpServer accept connections and handle data", "[ReliableUdpServer][ReliableUdpServer-accept-connections]") {
	std::unique_ptr<ReliableUdpServer> server = create_server(LOCALHOST, PORT);
	std::unique_ptr<ReliableUdpClient> client = create_client(LOCALHOST, PORT);
	...
	std::unique_ptr<ReliableUdpClient> client_from_server = accept_connection(server.get());
...
TEST_CASE("ReliableUdpServer handle multiple clients", "[ReliableUdpServer][ReliableUdpServer-multiple-clients]") {
	...
	std::vector<std::unique_ptr<ReliableUdpClient>> clients;
	PacketBuilder pb;
	for (int i = 0; i < 5; i++) {
		std::unique_ptr<ReliableUdpClient> c = create_client(LOCALHOST, PORT);
	...
	for (int i = 0; i < clients.size(); i++) {
		std::unique_ptr<ReliableUdpClient> cfs = accept_connection(server.get());
	...
	wait_for_condition([&]() {
		...
		for (std::unique_ptr<ReliableUdpClient>& c : clients) {
