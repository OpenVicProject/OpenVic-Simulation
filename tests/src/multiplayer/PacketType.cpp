...
#include "multiplayer/lowlevel/PacketBuilder.hpp"
...
inline static std::unique_ptr<HostManager> create_host(IpAddress const& p_address, int p_port) {
	std::unique_ptr<HostManager> host_manager = std::make_unique<HostManager>();
...
inline static std::unique_ptr<ClientManager> create_client(IpAddress const& p_address, int p_port) {
	std::unique_ptr<ClientManager> client = std::make_unique<ClientManager>();
...
TEST_CASE("PacketType update_host_session", "[PacketType][PacketType-update_host_session]") {
	std::unique_ptr<HostManager> host_manager = create_host(LOCALHOST, PORT);
	std::unique_ptr<ClientManager> client_manager = create_client(LOCALHOST, PORT);
	wait_for_condition([&]() {
