...
GoodDefinition good_definition {
	"test_good",   colour_rgb_t {}, good_index_t { 0 },
	good_category, base_price,      is_not_available_from_start,
	is_tradeable,  is_not_money,    does_not_counter_overseas_penalty
};
GameRulesManager game_rules_manager {};

struct Trader {
public:
	std::function<void(BuyResult const&)> buy_callback = [](BuyResult const& buy_result) -> void {
		FAIL("Did not expect buy callback");
	};
	std::function<void(SellResult const&, memory::vector<fixed_point_t>&)> sell_callback =
	    [](SellResult const& sell_result, memory::vector<fixed_point_t>& reusable_vector) -> void {
...
TEST_CASE("GoodMarket no trading when good isn't available", "[GoodMarket]") {
	...
	Trader buyer { .buy_callback = [](BuyResult const& buy_result) -> void {
	...
	good_market.add_buy_up_to_order({ country_index_optional, quantity_to_buy, money_to_spend, &buyer, Trader::after_buy });

	Trader seller { .sell_callback = [](SellResult const& sell_result, memory::vector<fixed_point_t>& reusable_vector) -> void {
	...
	good_market.add_market_sell_order({ country_index_optional, quantity_to_sell, &seller, Trader::after_sell });
	...
	good_market.execute_orders(reusable_country_map_0, reusable_country_map_1, reusable_vectors);
