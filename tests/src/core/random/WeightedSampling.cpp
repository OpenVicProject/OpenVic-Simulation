...
TEST_CASE("WeightedSampling limits", "[WeightedSampling]") {
	...
	for (size_t i = 0; i < weights.size(); ++i) {
		CHECK(sample_weighted_index(i * step_size, weights, weights_sum) == i);
	}
}
TEST_CASE("WeightedSampling weights", "[WeightedSampling]") {
	...
	for (size_t i = 0; i < weights.size(); ++i) {
		...
		const Int96DivisionResult random_value = portable_int96_div_int64(
			portable_int64_mult_uint32_96bit(cumulative_weight.get_raw_value(), max_random_value), weights_sum.get_raw_value()
		);
		REQUIRE_FALSE(random_value.quotient_overflow);
		CHECK(sample_weighted_index(static_cast<uint32_t>(random_value.quotient), weights, weights_sum) == i);
	}
}
