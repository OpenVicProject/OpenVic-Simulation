...
TEST_CASE("merge_extra_writes adds Write entries; existing R upgrades to W", "[ecs][SystemAccess]") {
	...
	REQUIRE(set.size() == 2u);
	// 100 upgrades from Read to Write; 200 gets a fresh Write.
	CHECK(set[0].component_id == 100);
	CHECK(set[0].mode == AccessMode::Write);
	CHECK(set[1].component_id == 200);
...
TEST_CASE("Same id in extra_reads and extra_writes coalesces to W in either merge order",
	...
	merge_extra_reads(reads_first, extras);
	merge_extra_writes(reads_first, extras);
	canonicalise_access_set(reads_first);

	std::vector<ComponentAccess> writes_first;
	merge_extra_writes(writes_first, extras);
	merge_extra_reads(writes_first, extras);
	canonicalise_access_set(writes_first);

	REQUIRE(reads_first.size() == 1u);
	CHECK(reads_first[0].component_id == 100);
	CHECK(reads_first[0].mode == AccessMode::Write);
	REQUIRE(writes_first.size() == 1u);
	CHECK(writes_first == reads_first);
}
