...
public:
	...
	// Default Constructor
	SpyAllocator() : metrics(std::make_shared<AllocationMetrics>()), upstream() {}

	// Explicitly pass an existing upstream allocator instance if needed
	explicit SpyAllocator(const Allocator& alloc) : metrics(std::make_shared<AllocationMetrics>()), upstream(alloc) {}
