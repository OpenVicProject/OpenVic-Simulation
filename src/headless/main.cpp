...
static void print_rgo(ProvinceInstance const& province) {
	...
	if (production_type_nullable == nullptr) {
		spdlog::error_s("\n\t{} - production_type: nullptr", province);
	}
#if SPDLOG_ACTIVE_LEVEL <= SPDLOG_LEVEL_INFO
	else {
		...
		for (auto [pop_type, employees_of_type] : rgo.get_employee_count_per_type_cache()) {
			if (employees_of_type > 0) {
				employees += fmt::format("\n\t\t{} {}", employees_of_type, pop_type);
			}
		}
		...
		SPDLOG_INFO(
			...
			"employees:{}",
			province, output_good, production_type, rgo.get_size_multiplier().to_string(3),
			rgo.get_output_quantity_yesterday().to_string(3), rgo.get_revenue_yesterday().to_string(3),
			rgo.get_total_owner_income_cache().to_string(3), rgo.get_total_employee_income_cache().to_string(3), employees
		);
	}
#endif
}
...
struct SinkGuard {
	...
	SinkGuard(spdlog::logger* l, spdlog::sink_ptr s) : logger(l), sink(s) {
