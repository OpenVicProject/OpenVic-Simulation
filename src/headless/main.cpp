...
static bool run_headless(Dataloader::path_vector_t const& roots, bool run_tests) {
	...
	if (game_manager.get_instance_manager()) {
		...
		for (int i = 0; i < std::min<int>(3, great_powers.size()); ++i) {
		...
		ProvinceInstance *const& new_orleans = game_manager.get_instance_manager()->get_map_instance().get_province_instance_by_identifier("139");
		print_rgo(*new_orleans);
	} else {
