...
inline static void print_memory_usage(
	Prefix const& prefix, std::source_location const& location = std::source_location::current()
) {
#ifdef DEBUG_ENABLED // memory tracking will return 0 without DEBUG_ENABLED
	Logger::info<Prefix const&, const char(&)[16], uint64_t&&, const char(&)[7]>(
...
static test_duration_t run_pathing_test(AStarPathing& pathing) {
...
static bool run_headless(fs::path const& root, memory::vector<memory::string>& mods, bool run_tests) {
	...
	if (ret) {
		...
		test_duration_t duration = std::chrono::duration_cast<test_time_units_t>(run_pathing_test<TESTS>(
			game_manager.get_instance_manager()->get_map_instance().get_land_pathing()
		));
		...
		duration = std::chrono::duration_cast<test_time_units_t>(run_pathing_test<TESTS>(
			game_manager.get_instance_manager()->get_map_instance().get_sea_pathing()
		));
	...
	if (ret) {
		...
		while (++ticks_passed < TICK_COUNT) {
			...
			print_memory_usage(memory::fmt::format("Tick ", ticks_passed, " finished"));
		}
		const test_time_point_t end_time = testing_clock_t::now();
