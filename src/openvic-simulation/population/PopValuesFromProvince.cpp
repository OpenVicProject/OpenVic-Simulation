...
void PopStrataValuesFromProvince::update_pop_strata_values_from_province(
    PopsDefines const& defines, ModifierEffectCache const& modifier_effect_cache, ProvinceInstance const& province
) {
	ModifierEffectCache::strata_effects_t const& strata_effects = modifier_effect_cache.get_strata_effects()[strata_index];
	fixed_point_t shared_base_needs_scalar =
	    defines.get_base_goods_demand() *
	...
	shared_everyday_needs_scalar =
	    shared_base_needs_scalar * invention_needs_scalar *
...
PopValuesFromProvince::PopValuesFromProvince(
    ...
    defines { new_defines }, effects_by_strata { generate_values, strata_size } {}
...
void PopValuesFromProvince::update_pop_values_from_province(ProvinceInstance& province) {
	...
	if (!ranked_artisanal_production_types.empty()) {
		std::stable_sort(
		    ranked_artisanal_production_types.begin(),
		    ranked_artisanal_production_types.end(),
