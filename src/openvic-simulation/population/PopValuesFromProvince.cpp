...
void PopStrataValuesFromProvince::update_pop_strata_values_from_province(
	PopsDefines const& defines, ModifierEffectCache const& modifier_effect_cache, Strata const& strata,
	...
	shared_life_needs_scalar =
		shared_base_needs_scalar * (fixed_point_t::_1 + province.get_modifier_effect_value(*strata_effects.get_life_needs()));
	shared_everyday_needs_scalar = shared_base_needs_scalar * invention_needs_scalar *
		(fixed_point_t::_1 + province.get_modifier_effect_value(*strata_effects.get_everyday_needs()));
	shared_luxury_needs_scalar = shared_base_needs_scalar * invention_needs_scalar *
...
PopValuesFromProvince::PopValuesFromProvince(
	GameRulesManager const& new_game_rules_manager, GoodInstanceManager const& new_good_instance_manager,
	ModifierEffectCache const& new_modifier_effect_cache, ProductionTypeManager const& new_production_type_manager,
	PopsDefines const& new_defines, decltype(effects_by_strata)::keys_span_type strata_keys
)
	: game_rules_manager { new_game_rules_manager }, good_instance_manager { new_good_instance_manager },
	  modifier_effect_cache { new_modifier_effect_cache }, production_type_manager { new_production_type_manager },
	  defines { new_defines }, effects_by_strata { strata_keys } {}
...
void PopValuesFromProvince::update_pop_values_from_province(ProvinceInstance& province) {
	...
	for (auto const& production_type : production_type_manager.get_production_types()) {
		const std::optional<fixed_point_t> estimated_score = ArtisanalProducer::estimate_production_type_score(
			good_instance_manager, production_type, province, max_cost_multiplier
		);
	...
	if (!ranked_artisanal_production_types.empty()) {
		std::stable_sort(
			ranked_artisanal_production_types.begin(), ranked_artisanal_production_types.end(),
