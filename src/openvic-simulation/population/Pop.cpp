...
fixed_point_t Pop::get_unemployment_fraction() const {
	...
	return fixed_point_t(type_safe::get(get_unemployed())) / size;
}
...
void Pop::update_gamestate(
	...
	if (type->can_be_recruited) {
		...
		if (
			...
			max_supported_regiments = (fixed_point_t(type_safe::get(size)) / (
				fixed_point_t(type_safe::get(military_defines.get_pop_size_per_regiment())) * pop_size_per_regiment_multiplier
			)).floor<size_t>() + 1;
		}
	}
}
