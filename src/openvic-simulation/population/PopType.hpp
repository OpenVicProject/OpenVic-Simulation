...
namespace OpenVic {
	...
	public:
		...
		PopType(
			std::string_view new_identifier, colour_t new_colour, index_t new_index, Strata const& new_strata,
			pop_sprite_t new_sprite, fixed_point_map_t<GoodDefinition const*>&& new_life_needs,
			...
			income_type_t new_everyday_needs_income_types, income_type_t new_luxury_needs_income_types,
			rebel_units_t&& new_rebel_units, pop_size_t new_max_size, pop_size_t new_merge_max_size,
			bool new_state_capital_only, bool new_demote_migrant, bool new_is_artisan, bool new_allowed_to_vote,
			bool new_is_slave, bool new_can_be_recruited, bool new_can_reduce_consciousness, bool new_is_administrator,
			bool new_can_invest, bool new_factory, bool new_can_work_factory, bool new_can_be_unemployed,
			fixed_point_t new_research_points, fixed_point_t new_leadership_points,
			fixed_point_t new_research_leadership_optimum, fixed_point_t new_state_administration_multiplier,
			PopType const* new_equivalent, ConditionalWeightFactorMul&& new_country_migration_target,
			ConditionalWeightFactorMul&& new_migration_target, poptype_weight_map_t&& new_promote_to,
			ideology_weight_map_t&& new_ideologies, issue_weight_map_t&& new_issues
		);
	...
	template<>
	struct enable_bitfield<PopType::income_type_t> : std::true_type {};
