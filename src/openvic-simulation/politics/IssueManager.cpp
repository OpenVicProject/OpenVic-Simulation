...
bool IssueManager::add_party_policy_group(std::string_view identifier) {
	...
	return party_policy_groups.emplace_item(
	    identifier, identifier, index_from_count<PartyPolicyGroup::index_t>(get_party_policy_group_count())
	);
}
...
bool IssueManager::add_party_policy(
    ...
    PartyPolicyGroup& party_policy_group,
    RuleSet&& rules,
    bool jingoism
) {
	if (identifier.empty()) {
	...
	if (!party_policies.emplace_item(
	        identifier,
	        index_from_count<PartyPolicy::index_t>(get_party_policy_count()),
	        identifier,
	        new_colour,
	        std::move(values),
	        party_policy_group,
...
bool IssueManager::add_reform_group(std::string_view identifier, ReformType& reform_type, bool ordered, bool administrative) {
	...
	if (!reform_groups.emplace_item(
	        identifier,
	        identifier,
	        index_from_count<ReformGroup::index_t>(get_reform_group_count()),
	        reform_type,
	        ordered,
	        administrative
	    )) {
...
bool IssueManager::add_reform(
    ...
    colour_t new_colour,
    ModifierValue&& values,
    ReformGroup& reform_group,
    size_t ordinal,
    fixed_point_t administrative_multiplier,
    RuleSet&& rules,
    Reform::tech_cost_t technology_cost,
    ConditionScript&& allow,
    ConditionScript&& on_execute_trigger,
	...
	if (!reforms.emplace_item(
	        identifier,
	        index_from_count<Reform::index_t>(get_reform_count()),
	        identifier,
	        new_colour,
	        std::move(values),
	        reform_group,
...
static constexpr colour_t create_issue_reform_colour(size_t index) {
	...
	ret[varying_colour_indices[periodic_index]] =
	    static_cast<size_t>(colour_t::max_value) * ((index * scattering_prime) % varying_colour_denominator) /
	    varying_colour_denominator;
...
bool IssueManager::_load_party_policy(
	...
	ret &= add_party_policy(
	    identifier,
	    create_issue_reform_colour(get_party_policy_count() + get_reform_count()),
	    std::move(values),
	    party_policy_group,
	    std::move(rules),
	    is_jingoism
