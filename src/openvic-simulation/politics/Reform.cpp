...
ReformGroup::ReformGroup(
    ...
    HasIdentifier { new_identifier }, HasIndex { new_index }, reform_type { new_reform_type }, is_ordered { new_is_ordered },
...
Reform::Reform(
    ...
    ReformGroup const& new_reform_group,
    size_t new_ordinal,
    fixed_point_t new_administrative_multiplier,
    RuleSet&& new_rules,
    tech_cost_t new_technology_cost,
    ConditionScript&& new_allow,
    ConditionScript&& new_on_execute_trigger,
    EffectScript&& new_on_execute_effect
) :
    BaseIssue { new_identifier, new_colour, std::move(new_values), std::move(new_rules), false, modifier_type_t::REFORM },
    HasIndex { new_index }, ordinal { new_ordinal }, administrative_multiplier { new_administrative_multiplier },
    technology_cost { new_technology_cost }, group { new_reform_group }, allow { std::move(new_allow) },
