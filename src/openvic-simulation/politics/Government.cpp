...
GovernmentType::GovernmentType(
	index_t new_index, std::string_view new_identifier, memory::vector<std::reference_wrapper<const Ideology>>&& new_ideologies,
	bool new_holds_elections, bool new_can_appoint_ruling_party, Timespan new_term_duration,
	std::string_view new_flag_type_identifier
)
	: HasIndex { new_index }, HasIdentifier { new_identifier }, ideologies { std::move(new_ideologies) },
	  holds_elections { new_holds_elections }, can_appoint_ruling_party { new_can_appoint_ruling_party },
	  term_duration { new_term_duration }, flag_type_identifier { new_flag_type_identifier } {}
...
bool GovernmentTypeManager::add_government_type(
	std::string_view identifier, memory::vector<std::reference_wrapper<const Ideology>>&& ideologies, bool elections,
	...
	const bool ret = government_types.emplace_item(
		identifier, GovernmentType::index_t { get_government_type_count() }, identifier, std::move(ideologies), elections,
		appoint_ruling_party, term_duration, flag_type
	);
...
bool GovernmentTypeManager::load_government_types_file(IdeologyManager const& ideology_manager, ast::NodeCPtr root) {
	...
	bool ret = expect_dictionary_reserve_length(government_types, [this, &ideology_manager](std::string_view government_type_identifier, ast::NodeCPtr government_type_value) -> bool {
		...
		bool
			ret =
				expect_dictionary_keys_and_default(increment_callback(total_expected_ideologies), "election", ZERO_OR_ONE, expect_bool(assign_variable_callback(elections)), "duration", ZERO_OR_ONE, expect_months(assign_variable_callback(term_duration)), "appoint_ruling_party", ONE_EXACTLY, expect_bool(assign_variable_callback(appoint_ruling_party)), "flagType", ZERO_OR_ONE, expect_identifier(assign_variable_callback(flag_type_identifier)))(
					government_type_value
				);
		ideologies.reserve(total_expected_ideologies);

		ret &=
			expect_dictionary([this, &ideology_manager, &ideologies, government_type_identifier](std::string_view key, ast::NodeCPtr value) -> bool {
				...
				if (ideology_ptr == nullptr) {
					spdlog::error_s(
						"When loading government type {}, specified ideology {} is invalid!", government_type_identifier, key
					);
					return false;
				}
				return expect_bool([&ideologies, &ideology = *ideology_ptr, government_type_identifier](bool val) -> bool {
					if (val) {
						if (!ranges::contains(ideologies, ideology)) {
							ideologies.emplace_back(ideology);
							return true;
						}
						spdlog::error_s(
							"Government type {} marked as supporting ideology {}", government_type_identifier, ideology
						);
						return false;
					}
					spdlog::error_s(
