...
NationalFocus::NationalFocus(
	std::string_view new_identifier, NationalFocusGroup const& new_group, uint8_t new_icon, bool new_has_flashpoint,
	fixed_point_t new_flashpoint_tension, bool new_own_provinces, bool new_outliner_show_as_percent,
	ModifierValue&& new_modifiers, Ideology const* new_loyalty_ideology, fixed_point_t new_loyalty_value,
	fixed_point_t new_encourage_railroads, fixed_point_map_t<GoodDefinition const*>&& new_encourage_goods,
	fixed_point_map_t<PopType const*>&& new_encourage_pop_types, ConditionScript&& new_limit
)
	: Modifier { new_identifier, std::move(new_modifiers), modifier_type_t::NATIONAL_FOCUS }, group { new_group },
	  icon { new_icon }, has_flashpoint { new_has_flashpoint }, flashpoint_tension { new_flashpoint_tension },
	  own_provinces { new_own_provinces }, outliner_show_as_percent { new_outliner_show_as_percent },
	  loyalty_ideology { new_loyalty_ideology }, loyalty_value { new_loyalty_value },
	  encourage_railroads { new_encourage_railroads }, encourage_goods { std::move(new_encourage_goods) },
	  encourage_pop_types { std::move(new_encourage_pop_types) }, limit { std::move(new_limit) } {}
...
inline bool NationalFocusManager::add_national_focus(
	std::string_view identifier, NationalFocusGroup const& group, uint8_t icon, bool has_flashpoint,
	fixed_point_t flashpoint_tension, bool own_provinces, bool outliner_show_as_percent, ModifierValue&& modifiers,
	Ideology const* loyalty_ideology, fixed_point_t loyalty_value, fixed_point_t encourage_railroads,
	fixed_point_map_t<GoodDefinition const*>&& encourage_goods, fixed_point_map_t<PopType const*>&& encourage_pop_types,
	...
	if ((loyalty_ideology == nullptr) != (loyalty_value == 0)) {
		spdlog::warn_s(
			"Party loyalty incorrectly defined for national focus {}: ideology = {}, value = {}", identifier,
			ovfmt::validate(loyalty_ideology), loyalty_value
		);
	}
...
bool NationalFocusManager::load_national_foci_file(
	...
	bool ret =
		expect_dictionary_reserve_length(national_focus_groups, [this, &expected_national_foci](std::string_view identifier, ast::NodeCPtr node) -> bool {
	...
	ret &= expect_national_focus_group_dictionary([this, &pop_manager, &ideology_manager, &good_definition_manager, &modifier_manager](NationalFocusGroup const& group, ast::NodeCPtr group_node) -> bool {
		return expect_dictionary([this, &group, &pop_manager, &ideology_manager, &good_definition_manager, &modifier_manager](std::string_view identifier, ast::NodeCPtr node) -> bool {
			spdlog::scope scope { fmt::format("national focus {}", identifier) };
			...
			ConditionScript limit { PROVINCE | COUNTRY, PROVINCE | COUNTRY, NO_SCOPE };
