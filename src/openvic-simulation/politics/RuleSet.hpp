...
namespace OpenVic {
	...
	enum struct ApportionmentMethod : std::uint8_t { largest_share, dhont, sainte_laque };
	enum struct UpperHouseComposition : std::uint8_t { same_as_ruling_party, rich_only, state_vote, population_vote };
	enum struct CulturalVotingRight : std::uint8_t { primary_culture_voting, culture_voting, all_voting };
...
#define DO_FOR_ALL_RULES_SEP(F, SEP) \
	F(build_factory) SEP F(expand_factory) \
	SEP F(open_factory) \
	SEP F(destroy_factory) \
	SEP F(build_railway) \
	SEP F(factory_priority) \
	SEP F(can_subsidise) \
	SEP F(pop_build_factory) \
	SEP F(pop_expand_factory) \
	SEP F(pop_open_factory) \
	SEP F(delete_factory_if_no_input) \
	SEP F(build_factory_invest) \
	SEP F(expand_factory_invest) \
	SEP F(open_factory_invest) \
	SEP F(build_railway_invest) \
	SEP F(can_invest_in_pop_projects) \
	SEP F(pop_build_factory_invest) \
	SEP F(pop_expand_factory_invest) \
	SEP F(pop_open_factory_invest) \
	SEP F(allow_foreign_investment) \
	SEP F(primary_culture_voting) \
	SEP F(culture_voting) \
	SEP F(all_voting) \
	SEP F(largest_share) \
	SEP F(dhont) \
	SEP F(sainte_laque) \
	SEP F(rich_only) \
	SEP F(same_as_ruling_party) \
	SEP F(state_vote) \
	SEP F(population_vote) \
	SEP F(slavery_allowed)

#define RULE_FIELDS(RULE) OptionalBool RULE : 2 = OptionalBool::UNSPECIFIED;
				...
				if (rules[i].value == OptionalBool::TRUE) {
					...
					for (size_t j = i + 1; j < N; ++j) {
						if (OV_unlikely(rules[j].value == OptionalBool::TRUE)) {
							spdlog::warn_s(
								"Both {} and {} are enabled for {}. {} is picked.", rules[i].name, rules[j].name, context_name,
								rules[i].name
							);
						}
					}
...
#define MAP_FROM_BOOL(RULE) \
	#RULE, ZERO_OR_ONE, NodeTools::expect_bool([&ruleset](const bool value) mutable -> bool { \
...
#define COMMA ,
		...
		static NodeTools::node_callback_t expect_rule_set(NodeTools::callback_t<RuleSet&&> ruleset_callback) {
			return [ruleset_callback](ast::NodeCPtr root) mutable -> bool {
				...
				bool outer_result = NodeTools::expect_dictionary_keys(DO_FOR_ALL_RULES_SEP(MAP_FROM_BOOL, COMMA))(root);
...
#undef ADD_IF_NOT_UNSPECIFIED
		...
		ApportionmentMethod get_apportionment_method() const {
			const RuleDefinition rules[] { DEF(largest_share), DEF(dhont), DEF(sainte_laque) };
			return select_with_priority<ApportionmentMethod>(rules);
		}
		UpperHouseComposition get_upper_house_composition() const {
			const RuleDefinition rules[] { DEF(same_as_ruling_party), DEF(rich_only), DEF(state_vote), DEF(population_vote) };
			return select_with_priority<UpperHouseComposition>(rules);
		}
		CulturalVotingRight get_cultural_voting_rights() const {
			const RuleDefinition rules[] { DEF(primary_culture_voting), DEF(culture_voting), DEF(all_voting) };
