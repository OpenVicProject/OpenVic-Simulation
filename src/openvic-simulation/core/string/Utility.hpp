...
namespace OpenVic {
	...
	inline memory::string ascii_tolower(std::string_view str) {
	...
	inline memory::string ascii_toupper(std::string_view str) {
		memory::string result { str };
		std::transform(result.begin(), result.end(), result.begin(), [](unsigned char c) -> unsigned char {
	...
	inline constexpr std::string_view get_filename(std::string_view filepath, std::string_view default_path) {
		std::string_view filename = get_filename(filepath);
		if (!filename.empty()) {
	...
	inline memory::string append_string_views(Args const&... args) {
		return _append_string_views(static_cast<std::string_view>(args)...);
	}
