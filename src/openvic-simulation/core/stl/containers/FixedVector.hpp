...
namespace OpenVic::stl {
	...
	private:
		...
		static constexpr T* allocate(Allocator allocator, const size_type capacity) {
			return capacity > size_type {} ? allocator_traits::allocate(allocator, get_index_as_size_t(capacity)) : nullptr;
		}
	...
	public:
		...
		constexpr explicit FixedVector(
		    const create_empty_t, const size_type capacity, std::type_identity_t<Allocator> const& alloc
		) : _max_size(capacity), _size(size_type {}), _allocator(alloc), _data_start_ptr(allocate(_allocator, capacity)) {}
		...
		constexpr FixedVector(
		    const size_type size, T const& value_for_all_indices, std::type_identity_t<Allocator> const& alloc
		) : _max_size(size), _size(size), _allocator(alloc), _data_start_ptr(allocate(_allocator, size)) {
		...
		requires(std::constructible_from<value_type, size_type> || std::is_default_constructible_v<value_type>)
		    :
		    FixedVector(size, [](const size_type i) -> value_type {
		...
		requires(!specialization_of<std::remove_cvref_t<std::invoke_result_t<GeneratorTemplateType, size_type>>, std::tuple>)
		         ...
		         && std::constructible_from<T, decltype(std::declval<GeneratorTemplateType>()(std::declval<size_type>()))>
		constexpr FixedVector(
		    const size_type size, GeneratorTemplateType&& generator, std::type_identity_t<Allocator> const& alloc
		) : _max_size(size), _size(size_type {}), _allocator(alloc), _data_start_ptr(allocate(_allocator, size)) {
			for (size_type i {}; i < size; ++i) {
				allocator_traits::construct(_allocator, begin() + get_index_as_size_t(i), generator(i));
				++_size;
			}
		}
				...
				std::apply(
				    [this, i](auto&&... args) {
					    allocator_traits::construct(
					        _allocator, begin() + get_index_as_size_t(i), std::forward<decltype(args)>(args)...
					    );
				    },
				    generator(i)
				);
		...
		constexpr iterator begin() OV_LIFETIME_BOUND {
			return _data_start_ptr;
		}
		constexpr const_iterator begin() const OV_LIFETIME_BOUND {
			return _data_start_ptr;
		}
		constexpr const_iterator cbegin() const OV_LIFETIME_BOUND {
			return _data_start_ptr;
		}

		constexpr iterator end() OV_LIFETIME_BOUND {
			return begin() + get_index_as_size_t(_size);
		}
		constexpr const_iterator end() const OV_LIFETIME_BOUND {
			return begin() + get_index_as_size_t(_size);
		}
		constexpr const_iterator cend() const OV_LIFETIME_BOUND {
			return cbegin() + get_index_as_size_t(_size);
		}

		using reverse_iterator = std::reverse_iterator<iterator>;
		using const_reverse_iterator = std::reverse_iterator<const_iterator>;

		constexpr reverse_iterator rbegin() OV_LIFETIME_BOUND {
			return reverse_iterator(end());
		}
		constexpr const_reverse_iterator rbegin() const OV_LIFETIME_BOUND {
			return const_reverse_iterator(end());
		}
		constexpr const_reverse_iterator crbegin() const OV_LIFETIME_BOUND {
			return const_reverse_iterator(end());
		}

		constexpr reverse_iterator rend() OV_LIFETIME_BOUND {
			return reverse_iterator(begin());
		}
		constexpr const_reverse_iterator rend() const OV_LIFETIME_BOUND {
			return const_reverse_iterator(begin());
		}
		constexpr const_reverse_iterator crend() const OV_LIFETIME_BOUND {
			return const_reverse_iterator(begin());
		}
		...
		constexpr iterator emplace_back(Args&&... args) OV_LIFETIME_BOUND {
			...
			allocator_traits::construct(_allocator, begin() + get_index_as_size_t(_size), std::forward<Args>(args)...);
			++_size;
			return end() - 1;
		}
		...
		constexpr void pop_back() {
			if (_size > size_type {}) {
				allocator_traits::destroy(_allocator, end() - 1);
				--_size;
			}
		}
		...
		constexpr void clear() {
			for (iterator it = end(); it != begin();) {
				--it;
				allocator_traits::destroy(_allocator, it);
			}
			_size = {};
		}
