...
namespace OpenVic {
	 ...
	 * `std::size_t getIndex() const` method.
	...
	struct IndexedFlatMap {
		using keys_span_type = OpenVic::forwardable_span<const KeyType>;
		using values_vector_type = std::conditional_t<
			std::is_move_constructible_v<ValueType> || std::is_copy_constructible_v<ValueType>, memory::vector<ValueType>,
	...
	private:
		 ...
		 */
		constexpr std::size_t get_internal_index_from_key(KeyType const& key) const {
			static_assert(has_get_index<KeyType>);
			const std::size_t index = key.get_index();
			if (index < min_index || index > max_index) {
				spdlog::error_s(
					"DEVELOPER: OpenVic::IndexedFlatMap<{},{}> attempted to access key with index {} which is outside the "
					"map's defined range [{}, {}].",
					type_name<KeyType>(), type_name<ValueType>(), index, min_index, max_index
				);
		...
		static bool validate_new_keys(keys_span_type new_keys) {
			...
			if (new_keys.empty()) {
				spdlog::warn_s(
					...
					type_name<KeyType>(), type_name<ValueType>()
				);
				return false;
			}

			const std::size_t min_index = new_keys.front().get_index();
			const std::size_t max_index = new_keys.back().get_index();
			const std::size_t expected_capacity = max_index - min_index + 1;

			if (new_keys.size() != expected_capacity) {
				spdlog::error_s(
					"DEVELOPER: OpenVic::IndexedFlatMap<{},{}> must be constructed with a continuous span of keys with "
					"incremental indices. Expected capacity {} but got {} keys.",
					type_name<KeyType>(), type_name<ValueType>(), expected_capacity, new_keys.size()
				);
			...
			for (std::size_t i = 0; i < new_keys.size(); ++i) {
				if (new_keys[i].get_index() != min_index + i) {
					spdlog::error_s(
						...
						type_name<KeyType>(), type_name<ValueType>(), min_index + i, new_keys[i].get_index(), i
					);
		...
		keys_span_type get_shared_keys( //
			...
			const std::size_t min_shared_index = std::max(other.get_min_index(), min_index);
			const std::size_t max_shared_index = std::min(other.get_max_index(), max_index);

			if (min_shared_index > max_shared_index) {
				return {};
			}

			const std::size_t shared_count = 1 + max_shared_index - min_shared_index;
			const std::size_t other_keys_offset = min_shared_index - other.get_min_index();
		...
		bool check_subset_span_match(IndexedFlatMap<KeyType, OtherValueType> const& other) const {
			...
			if (!(other.get_min_index() >= min_index && other.get_max_index() <= max_index)) {
				spdlog::error_s(
					...
					type_name<KeyType>(), type_name<ValueType>(), other.get_min_index(), other.get_max_index(), min_index,
					max_index
				);
			...
			// There is no check for keys.data() being identical as KeyType objects are considered functionally equivalent if
			// their get_index() values match.
			return true;
		}
	...
	public:
		 ...
		 * @param value_generator A callable that takes a `KeyType const&` and returns a single argument to construct
		 * `ValueType`. This is used to generate values for each key in the provided span.
		 */
		template<typename GeneratorTemplateType>
		requires(std::is_move_constructible_v<ValueType> || std::is_copy_constructible_v<ValueType>)
					// value_generator(key) doesn't return std:tuple<...>
					&& (!specialization_of<
						   std::remove_cvref_t<std::invoke_result_t<GeneratorTemplateType, KeyType const&>>, std::tuple>)
					// ValueType(value_generator(key)) is valid constructor call
					&& std::constructible_from<
						   ValueType, decltype(std::declval<GeneratorTemplateType>()(std::declval<KeyType const&>()))>
		IndexedFlatMap(keys_span_type new_keys, GeneratorTemplateType value_generator)
			: keys(new_keys), min_index { new_keys.front().get_index() }, max_index { new_keys.back().get_index() }, values() {
			static_assert(has_get_index<KeyType>);
		...
		requires(!std::is_move_constructible_v<ValueType>) &&
					...
					&& std::constructible_from<
						   ValueType, decltype(std::declval<GeneratorTemplateType>()(std::declval<KeyType const&>()))>
		IndexedFlatMap(keys_span_type new_keys, GeneratorTemplateType value_generator)
			: keys(new_keys), min_index { new_keys.front().get_index() }, max_index { new_keys.back().get_index() },
		...
		/** vector
		 * @brief Constructs an IndexedFlatMap based on a provided span of ordered and continuous keys.
		 * All elements are constructed by passing `KeyType const&` or default-constructed if there is no constructor taking
		 * `KeyType const&`.
		 * @param new_keys A `std::span<const KeyType>` of KeyType objects. These keys MUST be
		 * ordered by their index and continuous (i.e., `new_keys[i+1].getIndex() == new_keys[i].getIndex() + 1`).
		 * The `IndexedFlatMap` stores a reference to this span; the caller is responsible
		 * for ensuring the underlying data outlives this map instance.
		 *
		 * @note This constructor requires `ValueType` to be `std::default_initializable || std::constructible_from<ValueType,
		 * KeyType const&>`.
		 */
		...
		{
			static_assert(has_get_index<KeyType>);
			std::swap(values[get_internal_index_from_key(key)], value);
		}
		...
		constexpr ValueType& at_index(const std::size_t index) {
			if (index < min_index || index > max_index) {
				spdlog::error_s(
					"DEVELOPER: OpenVic::IndexedFlatMap<{},{}> attempted to access index {} which is outside the map's defined "
					"range [{}, {}].",
					type_name<KeyType>(), type_name<ValueType>(), index, min_index, max_index
				);
				assert(index >= min_index && index <= max_index);
		...
		constexpr ValueType const& at_index(const std::size_t index) const {
			if (index < min_index || index > max_index) {
				spdlog::error_s(
					"DEVELOPER: OpenVic::IndexedFlatMap<{},{}> attempted to access index {} which is outside the map's defined "
					"range [{}, {}].",
		...
		constexpr bool contains_index(const std::size_t external_index) const {
		...
		constexpr OpenVic::forwardable_span<ValueType> get_values() {
			return values;
		}

		constexpr OpenVic::forwardable_span<const ValueType> get_values() const {
			return values;
		}
		...
		constexpr std::size_t get_min_index() const {
			return min_index;
		}

		constexpr std::size_t get_max_index() const {
			return max_index;
		}
				...
				if (other.contains(key)) {
					if (other.at(key) == static_cast<ValueType>(0)) {
						spdlog::error_s(
							...
							type_name<KeyType>(), type_name<ValueType>(), key.get_index()
						);
		...
		requires divisible<ValueType, OtherValueType, ValueType>
		{
			...
			return IndexedFlatMap(keys, [&](KeyType const& key) {
				if (other.contains(key)) {
					if (other.at(key) == static_cast<ValueType>(0)) {
						return handle_div_by_zero(this->at(key), other.at(key));
					}
					return this->at(key) / other.at(key);
				} else {
					return this->at(key); // Retain original value if key not in 'other'
		...
		requires divisible<ValueType, ScalarType, ValueType>
		{
			...
			if (scalar == static_cast<ValueType>(0)) {
				spdlog::error_s(
					"DEVELOPER: OpenVic::IndexedFlatMap<{},{}> division by zero for scalar operation.", type_name<KeyType>(),
					type_name<ValueType>()
				);
		...
		requires divide_assignable<ValueType, OtherValueType>
		{
			...
			for (KeyType const& key : other.get_keys()) {
				if (other.at(key) == static_cast<ValueType>(0)) {
					handle_div_by_zero(this->at(key), other.at(key));
				} else {
					this->at(key) /= other.at(key);
				}
			}
		...
		constexpr IndexedFlatMap&
		mul_add(IndexedFlatMap<KeyType, ValueTypeA> const& a, IndexedFlatMap<KeyType, ValueTypeB> const& b)
		requires mul_add_assignable<ValueType, ValueTypeA, ValueTypeB>
		{
			...
			if (a.get_min_index() != b.get_min_index() || a.get_max_index() != b.get_max_index()) {
				spdlog::error_s(
					"DEVELOPER: OpenVic::IndexedFlatMap<{},{}> attempted mul_add where a and b don't have the same keys. This "
					"is not implemented.",
					type_name<KeyType>(), type_name<ValueType>()
				);
		...
		requires std::default_initializable<ValueType> && add_assignable<ValueType> &&
			multiply_assignable<ValueType, ScalarType> && divide_assignable<ValueType>
		{
			static_assert(has_get_index<KeyType>);
		...
		private:
			std::conditional_t<IsConst, typename values_vector_type::const_iterator, typename values_vector_type::iterator>
				value_it;
	...
	IndexedFlatMap<KeyType, ValueType> operator+(ScalarType const& scalar, IndexedFlatMap<KeyType, ValueType> const& map)
	requires addable<ValueType, ScalarType, ValueType>
	{
	...
	template<typename ScalarType, typename KeyType, typename ValueType>
	IndexedFlatMap<KeyType, ValueType> operator-(ValueType const& scalar, IndexedFlatMap<KeyType, ValueType> const& map)
