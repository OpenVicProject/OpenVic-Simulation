...
namespace OpenVic::memory {
	...
	FOONATHAN_ALIAS_TEMPLATE(deque, OpenVic::utility::deque<T, foonathan::memory::std_allocator<T, tracker<RawAllocator>>>);

	template<typename T, class RawAllocator = foonathan::memory::default_allocator>
	FOONATHAN_ALIAS_TEMPLATE(
	    deque_scoped_alloc,
	    OpenVic::utility::deque<T, std::scoped_allocator_adaptor<foonathan::memory::std_allocator<T, tracker<RawAllocator>>>>
	);
}
