...
#include "openvic-simulation/core/portable/ForwardableSpan.hpp" // IWYU pragma: keep
...
#define OV_DETAIL_GET_TYPE \
	...
	constexpr auto _self_type_helper() -> decltype(::OpenVic::Writer<_self_type_tag, decltype(this)> {}); \
	using type = ::OpenVic::Read<_self_type_tag>; \
	static constexpr std::string_view get_type_static() { \
...
\
public: \
	[[nodiscard]] constexpr auto get_##NAME() const->OpenVic::forwardable_span<const decltype(NAME)::value_type> { \
		return NAME; \
	} \
	ACCESS:
