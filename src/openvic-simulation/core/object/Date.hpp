...
namespace OpenVic {
	...
	public:
		...
		using year_t = Timespan::year_t;
		using month_t = Timespan::month_t;
		using day_t = Timespan::day_t;

		static constexpr std::array DAYS_IN_MONTH = Timespan::DAYS_IN_MONTH;

		static constexpr day_t MONTHS_IN_YEAR = Timespan::MONTHS_IN_YEAR;
		static constexpr day_t MAX_DAYS_IN_MONTH = *ranges::max_element(DAYS_IN_MONTH);

		static constexpr Timespan::value_t DAYS_IN_YEAR = Timespan::DAYS_IN_YEAR;
		static constexpr std::array DAYS_UP_TO_MONTH = Timespan::DAYS_UP_TO_MONTH;
		...
		static constexpr std::array<month_t, DAYS_IN_YEAR> MONTH_FROM_DAY_IN_YEAR = [] {
			...
			Timespan::value_t days_left = 0;
			month_t month = 0;
			for (Timespan::value_t day = 0; day < DAYS_IN_YEAR; day++) {
	...
	public:
		...
		OV_SPEED_INLINE constexpr day_t get_day() const {
			Timespan::value_t day_of_year = get_day_of_year();
		...
		OV_SPEED_INLINE constexpr day_t get_day_of_week() const {
			return (static_cast<Timespan::value_t>(timespan) + INITIAL_WEEKDAY_NAME) % WEEKDAY_NAMES.size();
		}
