...
struct fmt::formatter<T> {
	constexpr void set_brackets(fmt::basic_string_view<char> open, fmt::basic_string_view<char> close) {
		_opening_bracket = open;
		_closing_bracket = close;
	}

	constexpr void set_separator(fmt::basic_string_view<char> sep) {
		_separator = sep;
	}

	constexpr fmt::format_parse_context::iterator parse(fmt::format_parse_context& ctx) {
		fmt::format_parse_context::iterator it = ctx.begin(), end = ctx.end();
		if (it == end || *it == '}') {
			return it;
		}

		it = fmt::detail::parse_align(it, end, _specs);
		if (it == end) {
			return it;
		}
		...
		char c = *it;
		if ((c >= '0' && c <= '9') || c == '{') {
			it = fmt::detail::parse_width(it, end, _specs, _specs.width_ref, ctx);
			if (it == end) {
				return it;
			}
		}
		...
		case 'x':
			_specs.set_type(fmt::presentation_type::hex);
			++it;
			break;
		}
		if (it == end) {
		...
		if (it != end && (*it == ':' || _value_format_type != value_format_type::none)) {
			if (_specs.align() != fmt::align::none) {
			...
			if (_specs.width != 0 || _specs.dynamic_width() != fmt::arg_id_kind::none) {
				fmt::report_error("invalid format specifier");
			}
			...
			if (_specs.type() == fmt::presentation_type::hex) {
				fmt::report_error("invalid format specifier");
			}
	...
	fmt::format_context::iterator format(T const& colour, fmt::format_context& ctx) const {
		fmt::format_specs specs { _specs };
		if (_specs.dynamic()) {
			fmt::detail::handle_dynamic_spec(_specs.dynamic_width(), specs.width, _specs.width_ref, ctx);
		}
		...
		if (_value_format_type == value_format_type::none) {
			...
			if (_specs.type() == fmt::presentation_type::hex) {
				...
				if (!upper) {
					...
					return fmt::detail::write(out, fmt::string_view { lower.data(), i }, specs, ctx.locale());
				}
			} else if (_specs.alt()) {
				*out++ = '#';
			}

			return fmt::detail::write(out, fmt::string_view { result.data(), result.size() }, specs, ctx.locale());
		}
		...
		out = fmt::detail::copy<char>(_opening_bracket, out);
		size_t size = colour.size();
		if (size == 4) {
			switch (_alpha_handle_type) {
			case alpha_handle_type::argb:
				write_value(colour[--size]);
				out = fmt::detail::copy<char>(_separator, out);
				break;
		...
		return fmt::detail::copy<char>(_closing_bracket, out);
	}
...
private:
	...
	fmt::basic_string_view<char> _separator = fmt::detail::string_literal<char, ',', ' '> {};
	fmt::basic_string_view<char> _opening_bracket = fmt::detail::string_literal<char, '('> {};
	fmt::basic_string_view<char> _closing_bracket = fmt::detail::string_literal<char, ')'> {};
};
