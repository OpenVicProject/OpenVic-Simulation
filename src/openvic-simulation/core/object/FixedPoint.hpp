...
namespace OpenVic {
	...
	public:
		...
		OV_ALWAYS_INLINE constexpr fixed_point_t(int32_t new_value)
			: value { static_cast<value_type>(new_value) << PRECISION } {}
		...
		OV_SPEED_INLINE constexpr static fixed_point_t mul_div( //
			fixed_point_t const& a, fixed_point_t const& b, fixed_point_t const& denominator
		) {
	...
	private:
		...
		OV_SPEED_INLINE static constexpr std::from_chars_result from_chars_integer( //
			char const* str, char const* const end, fixed_point_t& value
		) {
		...
		OV_SPEED_INLINE static constexpr std::from_chars_result from_chars_fraction( //
			char const* begin, char const* end, fixed_point_t& value
		) {
			if (begin && *begin == '-') {
				return { begin, std::errc::invalid_argument };
			}
