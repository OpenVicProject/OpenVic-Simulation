...
namespace OpenVic::Vm {
	struct ProgramRef {
		...
		void link_modules(std::span<ModuleRef> mods) {
			std::vector<const lauf_asm_module*> mod_list = Module::make_list(mods);
			link_modules(mod_list);
		}
		void link_modules(std::span<const lauf_asm_module*> mods) {
			lauf_asm_link_modules(*this, mods.data(), mods.size());
		}
