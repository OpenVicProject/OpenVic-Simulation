...
bool Dataloader::_load_history(DefinitionManager& definition_manager, bool unused_history_file_warnings) const {
...

	{
		...
		const path_vector_t province_history_files = lookup_basic_identifier_prefixed_files_in_dir_recursive(
		    province_history_directory, ".txt"
		);
		...
		for (memory::string const& dir : pop_history_dirs) {
			...
			if (result.ec == std::errc {} && date <= last_bookmark_date) {
				...
				ret &= apply_to_files(
				    ...
				    [this, &definition_manager, &province_history_manager, date, &non_integer_size](fs::path const& file)
				        -> bool {
...
#undef APPLY_TO_MAP_PATHS
	...
	if (!map_definition.load_map_images(
	        ...
	        lookup_file(append_string_views(map_directory, rivers)),
	        false
	    )) {
...
bool Dataloader::load_defines(GameRulesManager const& game_rules_manager, DefinitionManager& definition_manager) {
	...
	if (!definition_manager.get_politics_manager().load_national_foci_file(
	        ...
	        definition_manager.get_economy_manager().get_good_definition_manager(),
	        definition_manager.get_modifier_manager(),
	        parse_defines_cached(lookup_file(national_foci_file)).get_file_node()
	    )) {
