...
bool Dataloader::set_roots(std::span<const fs::path> new_roots, std::span<const fs::path> new_replace_paths) {
	if (!roots.empty()) {
...
bool Dataloader::should_ignore_path(fs::path const& path, std::span<const fs::path> replace_paths) const {
	bool ignore = false;
	for (fs::path const& replace_path : replace_paths) {
...
bool Dataloader::apply_to_files(std::span<const fs::path> files, apply_files_callback_t callback) const {
	bool ret = true;
