...
node_callback_t ConditionalWeight<TYPE>::expect_conditional_weight() {
	...
	if constexpr (TYPE == BASE) {
		successfully_set_up_base_keys &= add_key_map_entry(
		    key_map, "base", ONE_EXACTLY, expect_fixed_point(assign_variable_callback(base))
		);
	} else if constexpr (TYPE == FACTOR_ADD || TYPE == FACTOR_MUL) {
		successfully_set_up_base_keys &= add_key_map_entry(
		    key_map, "factor", ONE_EXACTLY, expect_fixed_point(assign_variable_callback(base))
		);
		...
		successfully_set_up_base_keys &= add_key_map_entries(
		    ...
		    "days",
		    ZERO_OR_ONE,
		    expect_uint<uint32_t>(time_callback("days", Timespan::from_days)),
		    "months",
		    ZERO_OR_ONE,
		    expect_uint<uint32_t>(time_callback("months", Timespan::from_months)),
		    "years",
		    ZERO_OR_ONE,
		    expect_uint<uint32_t>(time_callback("years", [](Timespan::value_t v) {
			    return Timespan::from_years(v);
		    }))
		);
	} else {
	...
	return expect_dictionary_key_map(
	    ...
	    "modifier",
	    ZERO_OR_MORE,
	    expect_modifier(condition_weight_items, initial_scope, this_scope, from_scope),
	    "group",
	    ZERO_OR_MORE,
