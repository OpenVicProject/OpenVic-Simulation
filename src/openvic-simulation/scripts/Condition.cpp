...
Condition::Condition(
    ...
    value_type_t new_value_type,
    scope_type_t new_scope,
    scope_type_t new_scope_change,
    identifier_type_t new_key_identifier_type,
...
node_callback_t ConditionManager::expect_condition_node(
	...
	return [this, &definition_manager, &condition, callback, current_scope, this_scope, from_scope](ast::NodeCPtr node) mutable
	           -> bool {
		bool ret = false;
...
#undef EXPECT_PAIR
		...
		if (!share_scope_type(scope, effective_current_scope) && effective_current_scope > scope) {
			spdlog::warn_s(
			    ...
			    identifier,
			    fmt::underlying(effective_current_scope),
...
bool ConditionManager::expect_condition_script(
    ...
    std::span<const ast::NodeCPtr> nodes
) const {
