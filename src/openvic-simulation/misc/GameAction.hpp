...
#define ARG_TYPE(name) name##_argument_t
	...
	struct none_argument_t : type_safe::strong_typedef<none_argument_t, std::monostate> {
		...
		constexpr none_argument_t(std::monostate&& value) : strong_typedef(static_cast<std::monostate&&>(value)) {}
	};
...
#define COMMA_ARG_TYPE(name, ...) , ARG_TYPE(name)
	using game_action_t = std::variant<none_argument_t FOR_EACH_GAME_ACTION(COMMA_ARG_TYPE)>;
