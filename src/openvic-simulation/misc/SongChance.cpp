...
SongChance::SongChance(size_t new_index, std::string_view new_filename, ConditionalWeightFactorMul&& new_chance)
...
bool SongChanceManager::load_songs_file(ast::NodeCPtr root) {
	...
	ret &= expect_dictionary_reserve_length(song_chances, [this](std::string_view key, ast::NodeCPtr value) -> bool {
		...
		ret &= song_chances.emplace_item(name, song_chances.size(), name, std::move(chance));
		return ret;
	})(root);
