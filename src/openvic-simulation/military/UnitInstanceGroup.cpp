		...
		if (new_leader != nullptr) {
			...
			if (OV_unlikely(&new_leader->get_country() != country)) {
				spdlog::error_s(
					...
					new_leader->get_name(),
					new_leader->get_country().get_identifier(),
...
bool UnitInstanceManager::generate_unit_instance_group(
	...
	if (unit_deployment_group.get_units().empty()) {
		spdlog::error_s(
			...
			unit_deployment_group.get_name(), country.get_identifier()
		);
		return false;
	}
	...
	if (unit_deployment_group.get_leader_index().has_value()) {
		...
		if (it < leaders.end()) {
			...
			spdlog::error_s(
				...
				*unit_deployment_group.get_leader_index(), unit_deployment_group.get_name(), country.get_identifier()
			);
			ret = false;
		}
	}
...
bool UnitInstanceManager::generate_deployment(
	...
	if (deployment == nullptr) {
		spdlog::error_s("Trying to generate null deployment for {}", country.get_identifier());
		return false;
	}
