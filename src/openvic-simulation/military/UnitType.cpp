...
UnitType::UnitType(
    ...
    unit_type_index_t new_unit_type_index,
    unit_branch_t new_branch,
    unit_type_args_t& unit_args
) :
    HasIdentifier { new_identifier }, unit_type_index { new_unit_type_index }, branch { new_branch }, icon { unit_args.icon },
    sprite { unit_args.sprite }, starts_unlocked { unit_args.starts_unlocked }, unit_category { unit_args.unit_category },
...
UnitTypeBranched<LAND>::UnitTypeBranched(
    ...
    UnitType { new_identifier, new_unit_type_index, LAND, unit_args }, HasIndex { new_index },
    allowed_cultures { regiment_type_args.allowed_cultures }, sprite_override { regiment_type_args.sprite_override },
    sprite_mount { regiment_type_args.sprite_mount }, sprite_mount_attach_node { regiment_type_args.sprite_mount_attach_node },
...
UnitTypeBranched<NAVAL>::UnitTypeBranched(
    ...
    UnitType { new_identifier, new_unit_type_index, NAVAL, unit_args }, HasIndex { new_index },
    naval_icon { ship_type_args.naval_icon }, can_sail { ship_type_args.sail }, is_transport { ship_type_args.transport },
    is_capital { ship_type_args.capital }, colonial_points { ship_type_args.colonial_points },
    can_build_overseas { ship_type_args.build_overseas }, min_port_level { ship_type_args.min_port_level },
    limit_per_port { ship_type_args.limit_per_port }, supply_consumption_score { ship_type_args.supply_consumption_score },
    hull { ship_type_args.hull }, gun_power { ship_type_args.gun_power }, fire_range { ship_type_args.fire_range },
    evasion { ship_type_args.evasion }, torpedo_attack { ship_type_args.torpedo_attack } {}
...
bool UnitTypeManager::add_regiment_type(
	...
	bool ret = regiment_types.emplace_item(
	    ...
	    index_from_count<unit_type_index_t>(get_unit_type_count()),
	    identifier,
	    unit_args,
	    std::move(regiment_type_args)
	);
	if (ret) {
...
bool UnitTypeManager::load_unit_type_file(
    ...
    TerrainTypeManager const& terrain_type_manager,
	...
	return expect_dictionary([this, &good_definition_manager, &terrain_type_manager, &modifier_manager, &type_symbol](std::string_view key, ast::NodeCPtr value) -> bool {
		...
		ret &= add_key_map_entries(
		    ...
		    "icon",
		    ONE_EXACTLY,
		    expect_uint(assign_variable_callback(unit_args.icon)),
		    "type",
		    ONE_EXACTLY,
		    success_callback, /* Already loaded above using expect_key */
		    "sprite",
		    ONE_EXACTLY,
		    expect_identifier(assign_variable_callback(unit_args.sprite)),
		    "active",
		    ZERO_OR_ONE,
		    expect_bool(assign_variable_callback(unit_args.starts_unlocked)),
		    "unit_type",
		    ONE_EXACTLY,
		    expect_identifier(expect_mapped_string(unit_type_map, assign_variable_callback(unit_args.unit_category))),
		    "floating_flag",
		    ONE_EXACTLY,
		    expect_bool(assign_variable_callback(unit_args.floating_flag)),
		    "priority",
		    ONE_EXACTLY,
		    expect_uint(assign_variable_callback(unit_args.priority)),
		    "max_strength",
		    ONE_EXACTLY,
		    expect_fixed_point(assign_variable_callback(unit_args.max_strength)),
		    "default_organisation",
		    ONE_EXACTLY,
		    expect_fixed_point(assign_variable_callback(unit_args.default_organisation)),
		    "maximum_speed",
		    ONE_EXACTLY,
		    expect_fixed_point(assign_variable_callback(unit_args.maximum_speed)),
		    "weighted_value",
		    ONE_EXACTLY,
		    expect_fixed_point(assign_variable_callback(unit_args.weighted_value)),
		    "move_sound",
		    ZERO_OR_ONE,
		    expect_identifier(assign_variable_callback(unit_args.move_sound)),
		    "select_sound",
		    ZERO_OR_ONE,
		    expect_identifier(assign_variable_callback(unit_args.select_sound)),
		    "build_time",
		    ONE_EXACTLY,
		    expect_days(assign_variable_callback(unit_args.build_time)),
		    "build_cost",
		    ONE_EXACTLY,
		    good_definition_manager.expect_good_definition_decimal_map(move_variable_callback(unit_args.build_cost)),
		    "supply_consumption",
		    ONE_EXACTLY,
		    expect_fixed_point(assign_variable_callback(unit_args.supply_consumption)),
		    "supply_cost",
		    ONE_EXACTLY,
		    good_definition_manager.expect_good_definition_decimal_map(move_variable_callback(unit_args.supply_cost))
		);

		auto add_terrain_modifier_value =
		    [&unit_args,
		     &terrain_type_manager,
		     &modifier_manager](std::string_view default_key, ast::NodeCPtr default_value) -> bool {
		...
		case LAND: {
			...
			ret &= add_key_map_entries(
			    ...
			    "primary_culture",
			    ZERO_OR_ONE,
			    expect_bool(assign_variable_callback(is_restricted_to_primary_culture)),
			    "accepted_culture",
			    ZERO_OR_ONE,
			    expect_bool(assign_variable_callback(is_restricted_to_accepted_cultures)),
			    "sprite_override",
			    ZERO_OR_ONE,
			    expect_identifier(assign_variable_callback(regiment_type_args.sprite_override)),
			    "sprite_mount",
			    ZERO_OR_ONE,
			    expect_identifier(assign_variable_callback(regiment_type_args.sprite_mount)),
			    "sprite_mount_attach_node",
			    ZERO_OR_ONE,
			    expect_identifier(assign_variable_callback(regiment_type_args.sprite_mount_attach_node)),
			    "reconnaissance",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(regiment_type_args.reconnaissance)),
			    "attack",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(regiment_type_args.attack)),
			    "defence",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(regiment_type_args.defence)),
			    "discipline",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(regiment_type_args.discipline)),
			    "support",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(regiment_type_args.support)),
			    "maneuver",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(regiment_type_args.maneuver)),
			    "siege",
			    ZERO_OR_ONE,
		...
		case NAVAL: {
			...
			ret &= add_key_map_entries(
			    ...
			    "naval_icon",
			    ONE_EXACTLY,
			    expect_uint(assign_variable_callback(ship_type_args.naval_icon)),
			    "sail",
			    ZERO_OR_ONE,
			    expect_bool(assign_variable_callback(ship_type_args.sail)),
			    "transport",
			    ZERO_OR_ONE,
			    expect_bool(assign_variable_callback(ship_type_args.transport)),
			    "capital",
			    ZERO_OR_ONE,
			    expect_bool(assign_variable_callback(ship_type_args.capital)),
			    "colonial_points",
			    ZERO_OR_ONE,
			    expect_fixed_point(assign_variable_callback(ship_type_args.colonial_points)),
			    "can_build_overseas",
			    ZERO_OR_ONE,
			    expect_bool(assign_variable_callback(ship_type_args.build_overseas)),
			    "min_port_level",
			    ONE_EXACTLY,
			    expect_uint(assign_variable_callback(ship_type_args.min_port_level)),
			    "limit_per_port",
			    ONE_EXACTLY,
			    expect_int(assign_variable_callback(ship_type_args.limit_per_port)),
			    "supply_consumption_score",
			    ZERO_OR_ONE,
			    expect_fixed_point(assign_variable_callback(ship_type_args.supply_consumption_score)),
			    "hull",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(ship_type_args.hull)),
			    "gun_power",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(ship_type_args.gun_power)),
			    "fire_range",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(ship_type_args.fire_range)),
			    "evasion",
			    ONE_EXACTLY,
			    expect_fixed_point(assign_variable_callback(ship_type_args.evasion)),
			    "torpedo_attack",
			    ZERO_OR_ONE,
...
bool UnitTypeManager::generate_modifiers(ModifierManager& modifier_manager) const {
	...
	const auto generate_stat_modifiers =
		...
		const auto stat_modifier =
			...
			ret &= modifier_manager.register_technology_modifier_effect(
			    effect_cache,
			    ModifierManager::get_flat_identifier(identifier, suffix),
			    format,
	...
	regiment_type_effects = std::move(
	    decltype(ModifierEffectCache::regiment_type_effects) {
	        generate_values, regiment_type_index_t(get_regiment_type_count()) }
	);
	...
	ship_type_effects = std::move(
	    decltype(ModifierEffectCache::ship_type_effects) { generate_values, ship_type_index_t(get_ship_type_count()) }
	);
