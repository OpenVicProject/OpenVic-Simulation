...
void ResourceGatheringOperation::setup_location_ptr(ProvinceInstance& location) {
	if (location_ptr != nullptr) {
		spdlog::error_s("RGO already has a location_ptr pointing to province {}", location_ptr->get_identifier());
	}
...
fixed_point_t ResourceGatheringOperation::produce() {
	...
	if (owner.has_value()) {
		...
		if (state_ptr == nullptr) {
			spdlog::error_s("Province {} has no state.", location.get_identifier());
			return 0;
		}
		...
		if (total_owner_count_in_state_cache > 0) {
			switch (owner_job.get_effect_type()) {
				...
				case Job::effect_t::THROUGHPUT:
					...
					break;
				default:
					spdlog::error_s("Invalid job effect in RGO {}",production_type.get_identifier());
					break;
			}
		}
	}
...
void ResourceGatheringOperation::pay_employees(memory::vector<fixed_point_t>& reusable_vector) {
	...
	if (revenue <= fixed_point_t::_0 || total_worker_count_in_province_cache <= 0) {
		if (revenue < fixed_point_t::_0) {
			spdlog::error_s("Negative revenue for province {}", location.get_identifier());
		}
		if (total_worker_count_in_province_cache < 0) {
			spdlog::error_s("Negative total worker count for province {}", location.get_identifier());
		}
		return;
	}
