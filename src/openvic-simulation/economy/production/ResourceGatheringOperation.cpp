...
ResourceGatheringOperation::ResourceGatheringOperation(
	...
	: market_instance { rgo_deps.market_instance }, modifier_effect_cache { rgo_deps.modifier_effect_cache },
	  ...
	  employee_count_per_type_cache { rgo_deps.pop_type_keys } {}
...
void ResourceGatheringOperation::initialise_rgo_size_multiplier() {
	...
	if (size_modifier == 0) {
		...
		size_multiplier =
			((fixed_point_t::from_fraction<pop_sum_t>(total_worker_count_in_province, base_workforce_size) / size_modifier)
				 .ceil() *
			 fixed_point_t::_1_50)
				.floor();
	}
...
fixed_point_t ResourceGatheringOperation::calculate_size_modifier() const {
	...
	size_modifier +=
		location.get_modifier_effect_value(*modifier_effect_cache.get_good_effects(production_type.output_good).get_rgo_size());
	...
	if (owner.has_value()) {
		...
		if (total_owner_count_in_state_cache > 0) {
			...
			case Job::effect_t::OUTPUT:
				output_multiplier += owner_job.effect_multiplier.mul_div(total_owner_count_in_state_cache, state_population);
				break;
			case Job::effect_t::THROUGHPUT:
				throughput_multiplier +=
					owner_job.effect_multiplier.mul_div(total_owner_count_in_state_cache, state_population);
				break;
	...
	if (revenue <= total_minimum_wage) {
		for (Employee& employee : employees) {
			const fixed_point_t income_for_this_pop = std::max(
				fixed_point_t::mul_div(revenue, employee.get_minimum_wage_cached(), total_minimum_wage),
		...
		if (total_owner_count_in_state_cache > 0) {
			const fixed_point_t upper_limit =
				std::min(fixed_point_t::_0_50, fixed_point_t::_1 - total_minimum_wage / revenue_left);
