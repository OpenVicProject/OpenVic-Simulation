...
ArtisanalProducer::ArtisanalProducer(ArtisanalProducerDeps const& artisanal_producer_deps)
	: ArtisanalProducer(
		  artisanal_producer_deps, IndexedFlatMap<GoodDefinition, fixed_point_t> { artisanal_producer_deps.good_keys }, nullptr,
...
void ArtisanalProducer::artisan_tick_handler::calculate_inputs(
	...
	for (auto it = input_goods.begin(); it < input_goods.end(); it++) {
		...
		if (should_report_input_demand && country_to_report_economy_nullable != nullptr) {
			country_to_report_economy_nullable->report_input_demand(production_type, input_good, desired_quantity);
		}
...
void ArtisanalProducer::artisan_tick_handler::produce(
	...
	for (auto it = input_goods.begin(); it < input_goods.end(); it++) {
		...
		if (country_to_report_economy_nullable != nullptr) {
			country_to_report_economy_nullable->report_input_consumption(production_type, input_good, consumed_quantity);
		}
...
void ArtisanalProducer::artisan_tick_handler::allocate_money_for_inputs(
	...
	memory::vector<fixed_point_t>& pop_max_quantity_to_buy_per_good, memory::vector<fixed_point_t>& pop_money_to_spend_per_good,
	IndexedFlatMap<GoodDefinition, fixed_point_t> const& stockpile, PopValuesFromProvince const& values_from_province
) {
	...
	for (auto it = input_goods.begin(); it < input_goods.end(); it++) {
		...
		if (max_quantity_to_buy > 0) {
			const fixed_point_t optimal_quantity =
				fixed_point_t::mul_div(good_demand, max_possible_satisfaction_numerator, max_possible_satisfaction_denominator);
			const fixed_point_t money_to_spend =
				market_instance.get_max_money_to_allocate_to_buy_quantity(input_good, optimal_quantity - stockpiled_quantity);
...
void ArtisanalProducer::artisan_tick(
	...
	memory::vector<fixed_point_t>& pop_max_quantity_to_buy_per_good, memory::vector<fixed_point_t>& pop_money_to_spend_per_good,
	memory::vector<fixed_point_t>& reusable_map_0, memory::vector<fixed_point_t>& reusable_map_1,
	...
	if (old_production_type_ptr != nullptr) {
		...
		artisan_tick_handler tick_handler { country_to_report_economy_nullable,
											demand_per_input,
											market_instance,
											max_price_per_input,
											pop,
		...
		if (old_production_type_ptr == new_production_type_ptr) {
			tick_handler.allocate_money_for_inputs(
				max_quantity_to_buy_per_good, pop_max_quantity_to_buy_per_good, pop_money_to_spend_per_good, stockpile,
...
fixed_point_t ArtisanalProducer::add_to_stockpile(GoodDefinition const& good, const fixed_point_t quantity) {
	if (OV_unlikely(quantity < 0)) {
		spdlog::error_s("Attempted to add negative quantity {} of {} to artisan stockpile.", quantity, good);
		return 0;
	}
...
void ArtisanalProducer::subtract_from_stockpile(GoodDefinition const& good, const fixed_point_t sold_quantity) {
	if (OV_unlikely(sold_quantity < 0)) {
		spdlog::error_s("Attempted to subtract negative quantity {} of {} to artisan stockpile.", sold_quantity, good);
		return;
	}
	...
	if (OV_unlikely(stockpiled_quantity < sold_quantity)) {
		spdlog::error_s(
			"Attempted to subtract more {} of {} from artisan stockpile than it has {}.", sold_quantity, good,
			stockpiled_quantity
		);
...
fixed_point_t ArtisanalProducer::calculate_production_type_score(
	...
	return (k * fixed_point_t::mul_div(costs, costs, revenue) - (1 + k) * costs + revenue)
