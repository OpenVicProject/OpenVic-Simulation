...
ArtisanalProducer::ArtisanalProducer(ArtisanalProducerDeps const& artisanal_producer_deps) :
    ArtisanalProducer(
...
ArtisanalProducer::ArtisanalProducer(
    ...
    modifier_effect_cache { artisanal_producer_deps.modifier_effect_cache }, stockpile { std::move(new_stockpile) },
    production_type_nullable { nullptr }, last_produced_good { new_last_produced_good },
...
void ArtisanalProducer::artisan_tick_handler::calculate_inputs(
	...
	// inputs_bought_fraction uses base_desired_quantity as population size is cancelled in the production and input
	// calculations.
	...
	std::fill(wants_more_mask.begin(), wants_more_mask.end(), false);
	...
	for (auto it = input_goods.begin(); it < input_goods.end(); it++) {
		...
		const fixed_point_t desired_quantity = demand_per_input[i] =
		    base_desired_quantity * pop_size / production_type.base_workforce_size;
		if (desired_quantity == 0) {
			continue;
		}

		if (should_report_input_demand && country_to_report_economy_nullable != nullptr) {
			country_to_report_economy_nullable->report_input_demand(production_type, input_good.index, desired_quantity);
		}
		...
		max_price_per_input[i] = market_instance.get_max_next_price(input_good.index);
		wants_more_mask[input_good.index] = true;
		distinct_goods_to_buy++;
	}

	inputs_bought_fraction = Fraction { inputs_bought_numerator, inputs_bought_denominator };
}
...
void ArtisanalProducer::artisan_tick_handler::produce(
	...
	for (auto it = input_goods.begin(); it < input_goods.end(); it++) {
		...
		costs_of_production += consumed_quantity * market_instance.get_good_instance(input_good.index).get_price();

		if (country_to_report_economy_nullable != nullptr) {
			country_to_report_economy_nullable->report_input_consumption(production_type, input_good.index, consumed_quantity);
		}
		...
		good_stockpile = std::max(fixed_point_t::_0, good_stockpile - consumed_quantity);

		if (good_stockpile >= demand_per_input[i]) {
			wants_more_mask[input_good.index] = false;
			distinct_goods_to_buy--;
		}
	}
...
void ArtisanalProducer::artisan_tick_handler::allocate_money_for_inputs(
	...
	const fixed_point_t total_cash_to_spend =
	    pop.get_cash().get_copy_of_value() / values_from_province.get_max_cost_multiplier();
	...
	fixed_point_t max_possible_satisfaction_numerator = 1, max_possible_satisfaction_denominator = 1;
	...
	while (!at_or_below_optimum) {
		...
		for (auto it = input_goods.begin(); it < input_goods.end(); it++) {
			GoodDefinition const& input_good = *it.key();
			if (!wants_more_mask[input_good.index]) {
				continue;
			}
			const ptrdiff_t i = it - input_goods.begin();
		...
		for (auto it = input_goods.begin(); it < input_goods.end(); it++) {
			GoodDefinition const& input_good = *it.key();
			char& wants_more = wants_more_mask[input_good.index];
			if (!wants_more) {
				continue;
			}
			const ptrdiff_t i = it - input_goods.begin();
			...
			const fixed_point_t optimal_quantity = fp::mul_div(
			    demand_per_input[i], max_possible_satisfaction_numerator, max_possible_satisfaction_denominator
			);
	...
	for (auto it = input_goods.begin(); it < input_goods.end(); it++) {
		...
		if (max_quantity_to_buy > 0) {
			const fixed_point_t optimal_quantity = fp::mul_div(
			    good_demand, max_possible_satisfaction_numerator, max_possible_satisfaction_denominator
			);
			const fixed_point_t money_to_spend = market_instance.get_max_money_to_allocate_to_buy_quantity(
			    input_good.index, optimal_quantity - stockpiled_quantity
			);
...
void ArtisanalProducer::artisan_tick(
	...
	ProductionType const* new_production_type_ptr = pick_production_type(pop, values_from_province, random_number_generator);
	...
	for (auto const& [good, stockpiled_quantity] : stockpile) {
		...
		goods_to_sell[good.index] = stockpiled_quantity;
	}
...
fixed_point_t ArtisanalProducer::add_to_stockpile(const good_index_t good_index, const fixed_point_t quantity) {
	if (OV_unlikely(quantity < 0)) {
		spdlog::error_s("Attempted to add negative quantity {} of {} to artisan stockpile.", quantity, good_index);
		return 0;
	}

	GoodDefinition const& good = stockpile.get_key_at_index(good_index);
	...
	stockpile.at_index(good_index) += quantity_added_to_stockpile;
	max_quantity_to_buy -= quantity_added_to_stockpile;
	return quantity_added_to_stockpile;
}

void ArtisanalProducer::subtract_from_stockpile(const good_index_t good_index, const fixed_point_t sold_quantity) {
	if (OV_unlikely(sold_quantity < 0)) {
		spdlog::error_s("Attempted to subtract negative quantity {} of {} to artisan stockpile.", sold_quantity, good_index);
		return;
	}

	fixed_point_t& stockpiled_quantity = stockpile.at_index(good_index);
...
std::optional<fixed_point_t> ArtisanalProducer::estimate_production_type_score(
	...
	return calculate_production_type_score(estimated_revenue, estimated_costs, production_type.base_workforce_size);
}

fixed_point_t ArtisanalProducer::calculate_production_type_score(
    const fixed_point_t revenue, const fixed_point_t costs, const pop_size_t workforce
) {
	if (costs >= revenue) {
		return 0;
	}
	...
	return fp::mul_div(
	    (k * fp::mul_div(costs, costs, revenue) - (1 + k) * costs + revenue),
...
ProductionType const* ArtisanalProducer::pick_production_type(
	...
	if (production_type_nullable == nullptr || (revenue <= costs)) {
		...
		const fixed_point_t current_score = calculate_production_type_score(revenue, costs, pop.get_size());
		...
		for (auto it = ranked_artisanal_production_types.begin(); it < ranked_artisanal_production_types.end(); ++it) {
			...
			if (current_score > score_estimate) {
				if (i == 0) {
					...
					relative_score = ((current_score - score_estimate) / (previous_score_estimate - score_estimate) +
		...
		if (switch_chance >= 1) {
			...
			const size_t should_switch = sample_weighted_index(random_number_generator(), weights, weights_sum);
	...
	const size_t sample_index = sample_weighted_index(random_number_generator(), weights, weights_sum);
