...
namespace OpenVic {
	...
	private:
		...
		memory::ordered_map<Pop*, pop_size_t> PROPERTY(employees);
		memory::fixed_point_map_t<GoodDefinition const*> PROPERTY(stockpile);
	...
	public:
		FactoryProducer(
			...
			memory::ordered_map<Pop*, pop_size_t>&& new_employees, memory::fixed_point_map_t<GoodDefinition const*>&& new_stockpile,
