...
fixed_point_t MarketInstance::get_max_money_to_allocate_to_buy_quantity(
	GoodDefinition const& good_definition, const fixed_point_t quantity
) const {
...
void MarketInstance::place_buy_up_to_order(BuyUpToOrder&& buy_up_to_order) {
	...
	if (OV_unlikely(buy_up_to_order.max_quantity <= 0)) {
		spdlog::error_s("Received BuyUpToOrder for {} with max quantity {}", good, buy_up_to_order.max_quantity);
...
void MarketInstance::place_market_sell_order(
	MarketSellOrder&& market_sell_order, memory::vector<fixed_point_t>& reusable_vector
) {
	...
	if (OV_unlikely(quantity <= 0)) {
		spdlog::error_s("Received MarketSellOrder for {} with quantity {}", good, quantity);
	...
	if (good.is_money) {
		market_sell_order.call_after_trade(
			{ market_sell_order.good, quantity, quantity * country_defines.get_gold_to_worker_pay_rate() * good.base_price },
			reusable_vector
		);
		return;
	}
