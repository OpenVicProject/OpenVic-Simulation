	...
	if (market_sell_orders.empty()) {
		...
		if (is_selling_for_max_price) {
			...
			do {
				...
				for (size_t i = 0; i < buy_up_to_orders.size(); i++) {
					...
					distributed_supply = fp::mul_div(remaining_supply, purchasing_power_per_order[i], purchasing_power_sum);
			...
			for (size_t i = 0; i < buy_up_to_orders.size(); i++) {
				...
				const fixed_point_t quantity_bought = quantity_bought_per_order[i] = std::min(
				    buy_up_to_order.max_quantity, buy_up_to_order.money_to_spend / new_price
				);
		...
		if (quantity_traded_yesterday == supply_sum) {
			...
			for (GoodMarketSellOrder const& market_sell_order : market_sell_orders) {
				...
				market_sell_order.call_after_trade({ good_definition.index, quantity_sold, money_gained }, reusable_vectors[0]);
			}
		} else {
			...
			for (GoodMarketSellOrder const& market_sell_order : market_sell_orders) {
				...
				if (!country_index_optional.has_value()) {
					...
					quantity_sold_domestically =
					    total_bought_domestically >= total_domestic_supply
					        ? quantity_offered
					        : fp::mul_div(
					              quantity_offered,
	...
	for (size_t i = 0; i < buy_up_to_orders.size(); i++) {
		...
		if (quantity_bought == 0) {
			...
			buy_up_to_order.call_after_trade(
			    { good_definition.index, quantity_bought, money_spent_total, money_spent_on_imports }
			);
		}
	}
}
