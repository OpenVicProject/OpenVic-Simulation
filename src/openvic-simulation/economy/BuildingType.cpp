			...
			if (building_type.is_in_province()) {
				if (port_building_type == nullptr) {
					...
					spdlog::error_s(
						...
						building_type.get_identifier(), port_building_type->get_identifier()
					);
					ret = false;
				}
			} else {
				spdlog::error_s(
					"Building type {} is marked as a port, but is not a province building!", building_type.get_identifier()
				);
				ret = false;
			}
		}
