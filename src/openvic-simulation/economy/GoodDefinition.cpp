...
GoodDefinition::GoodDefinition(
    ...
    base_price { new_base_price }, is_available_from_start { new_is_available_from_start }, is_tradeable { new_is_tradeable },
    is_money { new_is_money }, counters_overseas_penalty { new_counters_overseas_penalty } {}
...
bool GoodDefinitionManager::add_good_category(std::string_view identifier, size_t expected_goods_in_category) {
	...
	if (good_categories.emplace_item(
	        identifier, identifier, index_from_count<GoodCategory::index_t>(get_good_category_count())
	    )) {
...
bool GoodDefinitionManager::add_good_definition(
	...
	if (is_tradeable == is_money) {
		spdlog::warn_s(
		    "Good {} has tradeable: {} and money: {}. Money goods are never tradeable. All other goods are tradeable. Setting "
		    "tradeable has no effect.",
		    identifier,
		    bool_to_yes_no(is_tradeable),
		    bool_to_yes_no(is_money)
		);
	}
	...
	if (good_definitions.emplace_item(
	        ...
	        identifier,
	        colour,
	        index_from_count<GoodDefinition::index_t>(get_good_definition_count()),
	        category,
	        base_price,
...
bool GoodDefinitionManager::load_goods_file(ast::NodeCPtr root) {
	...
	bool ret =
	    expect_dictionary_reserve_length(good_categories, [this, &total_expected_goods](std::string_view key, ast::NodeCPtr value) -> bool {
	...
	ret &= good_categories.expect_item_dictionary([this](GoodCategory& good_category, ast::NodeCPtr good_category_value) -> bool {
...
bool GoodDefinitionManager::generate_modifiers(ModifierManager& modifier_manager) const {
	...
	memory::FixedVector<ModifierEffectCache::good_effects_t, good_index_t>& good_effects =
	    modifier_manager.modifier_effect_cache.good_effects;
	good_effects = std::move(
	    decltype(ModifierEffectCache::good_effects) { generate_values, good_index_t(get_good_definition_count()) }
	);
	...
	for (GoodDefinition const& good : get_good_definitions()) {
		...
		const auto good_modifier =
		    [&modifier_manager, &ret, &good_identifier](
			...
			ret &= modifier_manager.register_technology_modifier_effect(
			    effect_cache,
			    ModifierManager::get_flat_identifier(name, good_identifier),
			    format,
			    localisation_key,
			    has_no_effect
			);
		...
		const auto make_production_localisation_suffix = [&good](std::string_view localisation_suffix) -> memory::string {
			return memory::fmt::format("${}$ ${}$", good, localisation_suffix);
		};

		good_modifier(
		    this_good_effects.rgo_goods_input,
		    "rgo_goods_input",
		...
		good_modifier(
		    this_good_effects.rgo_goods_output,
		    "rgo_goods_output",
		    FORMAT_x100_1DP_PC_POS,
		    make_production_localisation_suffix("TECH_OUTPUT")
		);
		good_modifier(
		    this_good_effects.rgo_goods_throughput,
		    "rgo_goods_throughput",
		    FORMAT_x100_1DP_PC_POS,
		    make_production_localisation_suffix("TECH_THROUGHPUT")
		);
		good_modifier(
		    this_good_effects.factory_goods_input,
		    "factory_goods_input",
		    FORMAT_x100_1DP_PC_NEG,
		    make_production_localisation_suffix("TECH_INPUT")
		);
		good_modifier(
		    this_good_effects.factory_goods_output,
		    "factory_goods_output",
		    FORMAT_x100_1DP_PC_POS,
		    make_production_localisation_suffix("TECH_OUTPUT")
		);
		good_modifier(
		    this_good_effects.factory_goods_throughput,
		    "factory_goods_throughput",
		    FORMAT_x100_1DP_PC_POS,
		    make_production_localisation_suffix("TECH_THROUGHPUT")
		);
		good_modifier(
		    this_good_effects.artisan_goods_input,
		    "artisan_goods_input",
		    FORMAT_x100_1DP_PC_NEG,
		    make_production_localisation_suffix("TECH_INPUT"),
		    HAS_NO_EFFECT
		...
		good_modifier(
		    this_good_effects.artisan_goods_output,
		    "artisan_goods_output",
		    FORMAT_x100_1DP_PC_POS,
		    make_production_localisation_suffix("TECH_OUTPUT"),
		    HAS_NO_EFFECT
		);
		good_modifier(
		    this_good_effects.artisan_goods_throughput,
		    "artisan_goods_throughput",
		    FORMAT_x100_1DP_PC_POS,
		    make_production_localisation_suffix("TECH_THROUGHPUT"),
		    HAS_NO_EFFECT
		);
		good_modifier(this_good_effects.rgo_size, "rgo_size", FORMAT_x100_2DP_PC_POS, memory::fmt::format("{}_RGO_SIZE", good));
	}
