...
GoodInstance::GoodInstance(GoodDefinition const* new_good_definition, GameRulesManager const* new_game_rules_manager)
	: HasIdentifierAndColour { *new_good_definition }, HasIndex { new_good_definition->index },
...
GoodInstanceManager::GoodInstanceManager(
	...
	: good_definition_manager { new_good_definition_manager },
	  good_instance_by_definition { new_good_definition_manager.get_good_definitions(),
									[game_rules_manager_ptr =
										 &game_rules_manager](GoodDefinition const& good_definition) -> auto {
...
GoodInstance* GoodInstanceManager::get_good_instance_by_identifier(std::string_view identifier) {
	...
	return good_definition == nullptr ? nullptr : &get_good_instance_by_definition(*good_definition);
}
...
GoodInstance* GoodInstanceManager::get_good_instance_by_index(typename GoodInstance::index_t index) {
	return good_instance_by_definition.contains_index(index) ? &good_instance_by_definition.at_index(index) : nullptr;
}
