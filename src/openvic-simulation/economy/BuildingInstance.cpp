...
BuildingInstance::BuildingInstance(BuildingType const& new_building_type, building_level_t new_level) :
    HasIdentifier { new_building_type }, building_type { new_building_type }, level { new_level } {}
...
bool BuildingInstance::expand(
    ModifierEffectCache const& modifier_effect_cache, CountryInstance& actor, ProvinceInstance const& location
) {
	if (expansion_state != ExpansionState::CanExpand) {
		return false;
	}

	if (!building_type.can_be_built_in(modifier_effect_cache, level + 1, actor, location)) {
		return false;
	}
...
void BuildingInstance::update_gamestate(Date today) {
	...
	case ExpansionState::Expanding:
		expansion_progress = fixed_point_t {
			static_cast<int32_t>((today - start_date).to_int())
