...
bool MapDefinition::add_province_definition(std::string_view identifier, colour_t colour) {
	...
	if (province_number != ProvinceDefinition::NULL_PROVINCE_NUMBER) {
		spdlog::error_s(
		    ...
		    get_province_definition_from_number(province_number)->to_string(),
		    identifier
		);
		return false;
	}
...
ProvinceDefinition::distance_t MapDefinition::calculate_distance_between(
	...
	return fp::sqrt(fvec2_t { min_x, to_pos.y - from_pos.y }.length_squared());
}
...
bool MapDefinition::add_special_adjacency(
    ProvinceDefinition& from,
    ProvinceDefinition& to,
    adjacency_t::type_t type,
    ProvinceDefinition const* through,
	...
	const auto add_adjacency =
	    [this, distance, type, through, data](ProvinceDefinition& from, ProvinceDefinition const& to) -> bool {
		const memory::vector<adjacency_t>::iterator existing_adjacency = std::find_if(
		    from.adjacencies.begin(), from.adjacencies.end(), [&to](adjacency_t const& adj) -> bool {
			...
			if (type == IMPASSABLE) {
				...
				if (type != STRAIT && type != CANAL) {
					spdlog::error_s(
					    ...
					    from,
					    to,
					    adjacency_t::get_type_name(existing_adjacency->get_type()),
					    adjacency_t::get_type_name(type)
					);
					return false;
				}
...
void MapDefinition::_trace_river(BMP& rivers_bmp, ivec2_t start, river_t& river) {
	...
	while (!stack.empty()) {
		...
		// Force heap initialiser elision
		memory::vector<ivec2_t> points { segment.point };
		uint8_t size = river_data[segment.point.x + segment.point.y * rivers_bmp.get_width()] -
		               1; // determine river size by colour
...
bool MapDefinition::load_map_images(
	...
	if (terrain_bmp.get_bits_per_pixel() != expected_terrain_rivers_bpp) {
		spdlog::error_s(
		    ...
		    terrain_bmp.get_bits_per_pixel(),
	...
	if (province_bmp.get_width() != terrain_bmp.get_width() || province_bmp.get_height() != terrain_bmp.get_height() ||
		...
		spdlog::error_s(
		    ...
		    province_bmp.get_width(),
		    province_bmp.get_height(),
		    terrain_bmp.get_width(),
		    terrain_bmp.get_height(),
		    rivers_bmp.get_width(),
	...
	memory::FixedVector<fixed_point_map_t<TerrainType const*>, province_index_t> terrain_type_pixels_list(
	    province_count, [](const province_index_t i) {
	...
	memory::FixedVector<fixed_point_t, province_index_t> pixels_per_province(province_count, [](const province_index_t i) {
...
bool MapDefinition::load_continent_file(ModifierManager const& modifier_manager, ast::NodeCPtr root) {
	bool ret = expect_dictionary_reserve_length(continents, [this, &modifier_manager](std::string_view identifier, ast::NodeCPtr node) -> bool {
		...
		bool ret = NodeTools::
		    expect_dictionary_keys_and_default(modifier_manager.expect_base_province_modifier(values), "provinces", ONE_EXACTLY, expect_list_reserve_length(prov_list, expect_province_definition_identifier([&prov_list](ProvinceDefinition const& province) -> bool {
			                                                                                                                                                    if (
			                                                                                                                                                        province
			                                                                                                                                                            .continent ==
			                                                                                                                                                        nullptr
				                                                                                                                                                    ...
				                                                                                                                                                    spdlog::warn_s(
				                                                                                                                                                        "Province {} found in multiple continents",
				                                                                                                                                                        province
				                                                                                                                                                    );
			                                                                                                                                                    }
			                                                                                                                                                    return true;
