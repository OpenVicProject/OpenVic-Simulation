	...
	for (ProvinceDefinition const* const province_definition_ptr : provinces) {
		...
		if (OV_unlikely(province_definition.has_region())) {
			spdlog::warn_s(
				...
				province_definition.get_region()->get_identifier(),
				identifier
			);
	...
	if (lines.empty()) {
		...
		if (!_validate_province_definitions_header(header)) {
			spdlog::error_s(
				...
				fmt::streamed(header) // TODO: remove streamed
			);
		}
				...
				if (!identifier.empty()) {
					...
					if (!_parse_province_colour(colour, { line.get_value_for(1), line.get_value_for(2), line.get_value_for(3) })) {
						spdlog::error_s(
							...
							fmt::streamed(line) // TODO: remove streamed
						);
						ret = false;
					}
						...
						if (province.climate != cur_climate) {
							...
							if (province.climate != nullptr) {
								...
								spdlog::warn_s(
									...
									province.get_identifier(), identifier, old_climate->get_identifier()
								);
							}
							...
							spdlog::warn_s(
								...
								province.get_identifier(), identifier
							);
						}
						return true;
					}
