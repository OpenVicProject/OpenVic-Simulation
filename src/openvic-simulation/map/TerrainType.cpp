...
bool TerrainTypeManager::generate_modifiers(ModifierManager& modifier_manager) const {
	...
	unit_terrain_effects = std::move(
	    decltype(ModifierEffectCache::unit_terrain_effects) { generate_values, terrain_type_index_t(get_terrain_type_count()) }
	);
	...
	for (TerrainType const& terrain_type : get_terrain_types()) {
		...
		ret &= modifier_manager.register_unit_terrain_modifier_effect(
		    this_unit_terrain_effects.attack,
		    ModifierManager::get_flat_identifier("attack", identifier),
		    FORMAT_x100_1DP_PC_POS,
		    "UA_ATTACK"
		);
		ret &= modifier_manager.register_unit_terrain_modifier_effect(
		    this_unit_terrain_effects.defence,
		    ModifierManager::get_flat_identifier("defence", identifier),
		    FORMAT_x100_1DP_PC_POS,
		    "UA_DEFENCE"
		);
		ret &= modifier_manager.register_unit_terrain_modifier_effect(
		    this_unit_terrain_effects.movement,
		    ModifierManager::get_flat_identifier("movement", identifier),
		    FORMAT_x100_1DP_PC_POS,
		    "UA_MOVEMENT"
		);
		ret &= modifier_manager.register_unit_terrain_modifier_effect(
		    this_unit_terrain_effects.attrition,
		    ModifierManager::get_flat_identifier("attrition", identifier),
		    FORMAT_x1_1DP_PC_NEG,
		    "UA_ATTRITION"
		);
	}
	return ret;
...
bool TerrainTypeManager::add_terrain_type(
	...
	return terrain_types.emplace_item(
	    identifier,
	    index_from_count<TerrainType::index_t>(get_terrain_type_count()),
	    identifier,
	    colour,
	    std::move(values),
	    movement_cost,
	    defence_bonus,
	    combat_width_percentage_change,
	    is_water
	);
}
