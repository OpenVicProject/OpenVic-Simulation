...
bool MapmodeManager::add_mapmode(
	...
	return mapmodes.emplace_item(
	    identifier,
	    identifier,
	    index_from_count<Mapmode::index_t>(get_mapmode_count()),
	    colour_func,
	    localisation_key,
...
bool MapmodeManager::setup_mapmodes(MapDefinition const& map_definition, BuildingTypeManager const& building_type_manager) {
		...
		ret &= add_mapmode(
		    ...
		    [&building_type_manager, infrastructure_building_type_ptr](
			    ...
			    const colour_argb_t::value_type val = colour_argb_t::colour_traits::component_from_fraction(
			        type_safe::get(infrastructure.get_level()),
			        type_safe::get(infrastructure_building_type.max_level) + 1,
			        0.5f,
			        1.0f
			    );
	       ...
	       ProvinceInstance const* selected_province) -> Mapmode::base_stripe_t {
		    ...
		    if (!province.province_definition.is_water()) {
			    const colour_argb_t::value_type val = colour_argb_t::colour_traits::component_from_fraction(
			        ...
			        type_safe::get(map_instance.get_highest_province_population()) + 1,
			        0.1f,
			        1.0f
			    );
	...
	*/
	if constexpr (false) {
		...
		ret &= add_mapmode(
		    "mapmode_religion", shaded_mapmode<Religion, pop_sum_t>(&ProvinceInstance::get_population_by_religion)
		);
		    ...
		    [](MapInstance const& map_instance,
		       ...
		       ProvinceInstance const* selected_province) -> Mapmode::base_stripe_t {
			    if (selected_province != nullptr) {
				    ...
				    ProvinceDefinition::adjacency_t const* adj = selected_province_definition.get_adjacency_to(
				        province_definition
				    );
