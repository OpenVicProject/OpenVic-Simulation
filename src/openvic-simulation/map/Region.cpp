...
bool ProvinceSet::add_province(ProvinceDefinition const* province) {
	...
	if (contains_province(province)) {
		spdlog::warn_s("Cannot add province {} to province set - already in the set!", province->get_identifier());
		return false;
	}
...
bool ProvinceSet::remove_province(ProvinceDefinition const* province) {
	...
	if (it == provinces.end()) {
		spdlog::warn_s("Cannot remove province {} from province set - already not in the set!", province->get_identifier());
		return false;
	}
