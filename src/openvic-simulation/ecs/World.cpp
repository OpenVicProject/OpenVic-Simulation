...
bool World::in_tick_or_log_(char const* fn_name) {
	...
	if (in_tick_ && !in_apply_phase_) {
		// Loud diagnostic but no crash — preserves test stability. The dev/release split
		// of "assert vs log" is currently unified to log + early-return; tests can flip a
		// flag if they need the assert behaviour. For now this fits the codebase pattern.
		(void) fn_name;
		return true;
	}
	return false;
