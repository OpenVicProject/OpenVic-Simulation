...
namespace {
	...
	// Two systems whose access overlaps on a Write can STILL be co-scheduled when they
	// provably never iterate the same entity — so the data-dependency edge between them can
	// be dropped. A system matches archetype A iff require ⊆ A and exclude ∩ A = ∅.
	//
	//   Rule 1 (disjoint iteration): some component one system REQUIRES is EXCLUDED by the
	//     other (sorted_intersects(require_a, exclude_b) || sorted_intersects(require_b,
	//     exclude_a)). Then no archetype can satisfy both queries — the witness archetype
	//     require_a ∪ require_b is rejected by whichever side excludes the shared id. This is
	//     the COMPLETE characterisation of disjointness provable from (require, exclude) alone.
	//
	//   Rule 2 (conflict is archetype-iterated only): every conflicting component is in BOTH
	//     systems' require sets AND in NEITHER system's extra_reads/extra_writes. Require
	//     membership alone is necessary but NOT sufficient — a system may declare the same id
	//     in its tick pack AND in an extra list (it reads/writes its own row plus arbitrary
	//     other rows), in which case the access reaches rows outside its iteration and
	//     disjoint iteration proves nothing. Only when the conflicting access is purely the
	//     tick parameter pack does the shared access land strictly on each system's own
	//     iterated rows → disjoint chunks → no shared (entity, component) cell. A conflict
	//     reached ONLY via extra_reads/extra_writes (e.g. a singleton id) is not in require,
	//     fails this rule, and correctly keeps the edge.
	//
	// Type-level only (require/exclude/access metadata, never live archetypes), so the
	// schedule stays a pure function of the registered system types — the multiplayer
	// schedule_hash invariant. The degenerate self-contradiction case (a system requiring and
	// excluding the same id, matching nothing) fails Rule 1 and stays serialised: a harmless
	// no-op, deliberately not special-cased.
	bool sorted_contains(std::vector<component_type_id_t> const& v, component_type_id_t c) {
...
void SystemScheduler::rebuild(std::vector<SystemRegistration>& registry) {
	...
	if (!dag.recompute_depths_or_detect_cycle()) {
		// Declared cycle — fatal. Clear the schedule and bail (no stages, schedule_hash
		// 0); tick_systems runs nothing until the cycle is fixed. No error is logged.
		built_ = false;
		stages_.clear();
		schedule_hash_ = 0;
		return;
	...
	for (uint32_t i = 0; i < N; ++i) {
		...
		for (uint32_t j = i + 1; j < N; ++j) {
			...
			// the two systems provably iterate disjoint archetypes (e.g. one writes C and
			// reads B, the other writes C `Without<B>`). Co-schedule them — no edge.
			if (provably_disjoint(registry[i], registry[j])) {
				continue;
			}
...
std::size_t SystemScheduler::stage_index_of(
	...
	for (std::size_t s = 0; s < stages_.size(); ++s) {
		for (uint32_t reg_idx : stages_[s].registration_indices) {
			if (reg_idx < registry.size() && registry[reg_idx].type_id == type_id) {
				return s;
			}
		}
	}
...
void SystemScheduler::run(
	...
	for (ScheduledStage const& stage : stages_) {
		...
		// Evaluate optional should_run predicates — EXACTLY ONCE per system per tick
		// (each registration belongs to exactly one stage), on the main thread, at stage
		// start, before any of the stage's systems execute and before any work items or
		// query-cache prewarm. Stage-top evaluation (not lazy per system inside the serial
		// branch) keeps the observable semantics identical across serial_mode,
		// single-system stages, and the multi-system parallel branch: a predicate always
		// sees the world as of the previous stage's barrier, never a co-staged serial
		// system's mid-stage singleton writes. run_flags parallels
		// stage.registration_indices.
		std::vector<uint8_t> run_flags(stage.registration_indices.size(), 1u);
		for (std::size_t i = 0; i < stage.registration_indices.size(); ++i) {
			SystemRegistration& reg = registry[stage.registration_indices[i]];
			if (!reg.alive || reg.tick_all_fn == nullptr) {
				continue; // dead slot — the consumer loops below re-check and skip it
			}
			if (reg.should_run_fn != nullptr) {
				TickContext ctx { world, today, *reg.pending_cmd };
				run_flags[i] = reg.should_run_fn(ctx) ? 1u : 0u;
			}
		}
		...
		if (serial_mode || stage.registration_indices.size() == 1) {
			for (std::size_t i = 0; i < stage.registration_indices.size(); ++i) {
				SystemRegistration& reg = registry[stage.registration_indices[i]];
				// A skipped system's tick_all_fn is never called — for a SystemThreaded in
				// a single-system stage that means no collect_matching_chunks and no
				// parallel_for at all (both live inside dispatch_threaded).
				if (!reg.alive || reg.tick_all_fn == nullptr || run_flags[i] == 0u) {
					continue;
				}
			...
			// A should_run-skipped system's prewarm is skipped too: it contributes zero
			// work items below, so no worker ever looks its key up inside the parallel
			// section; a running co-staged system that happens to share an identical
			// (require, exclude) key prewarms it itself. The skip decision depends only
			// on the (deterministic) predicate, so this is identical at every worker
			// count.
			for (std::size_t i = 0; i < stage.registration_indices.size(); ++i) {
				SystemRegistration& reg = registry[stage.registration_indices[i]];
				if (!reg.alive || run_flags[i] == 0u) {
					continue;
				}
			...
			for (std::size_t flag_idx = 0; flag_idx < stage.registration_indices.size(); ++flag_idx) {
				uint32_t const reg_idx = stage.registration_indices[flag_idx];
				SystemRegistration& reg = registry[reg_idx];
				// A should_run-skipped system contributes NO work items: no collect_chunks,
				// no per-chunk CB resize/clear/arm, and crucially no threaded_systems entry —
				// its per-chunk CB slots were not armed this tick, so the post-join merge
				// pass below must not touch them. Its (empty) pending_cmd is still applied
				// at the stage barrier in the stage's deterministic emit order — harmless
				// and uniform.
				if (!reg.alive || reg.tick_all_fn == nullptr || run_flags[flag_idx] == 0u) {
					continue;
				}
		...
		// Stage barrier: apply each system's pending CommandBuffer in the stage's
		// deterministic emit order — stage.registration_indices as emitted by the Phase 5
		// topological sort, i.e. ascending system_type_id_t within the stage, independent
		// of registration order. World's in_tick_ flag is still true here (we only clear it
