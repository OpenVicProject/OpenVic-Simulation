...
namespace OpenVic {
	...
	struct BasicStreamPacketClient : PacketClient {
		...
		int64_t available_packets() const override {
			...
			while (remaining >= 4) {
				...
				uint32_t length = utility::decode<uint32_t>(lbuf);
				remaining -= 4;
				offset += 4;
				if (length > remaining) {
					break;
	...
	private:
		...
		NetworkError _get_packet(std::span<uint8_t>& r_set) override {
			...
			_ring_buffer.erase(_ring_buffer.begin(), 4); // get rid of first 4 bytes
		...
		mutable std::vector<uint8_t> _input_buffer;
		mutable std::vector<uint8_t> _output_buffer;
	};
