...
#include "openvic-simulation/multiplayer/NetworkSocketBase.hpp"
...
WindowsSocket WindowsSocket::accept(IpAddress& r_ip, port_type& r_port) {
	OV_ERR_FAIL_COND_V(!is_open(), {});
...
std::unique_ptr<NetworkSocketBase> WindowsSocket::virtual_accept(IpAddress& r_ip, port_type& r_port) {
	WindowsSocket socket = accept(r_ip, r_port);

	if (!socket.is_open()) {
		return nullptr;
	}

	return std::make_unique<WindowsSocket>(std::move(socket));
}
...
NetworkError WindowsSocket::change_multicast_group(IpAddress const& p_multi_address, std::string_view p_if_name, bool p_add) {
	...
	for (std::pair<std::string, NetworkResolver::InterfaceInfo> const& pair : if_info) {
