...
namespace OpenVic {
	struct UdpServer : PacketServer {
		...
		std::unique_ptr<UdpClient> take_next_client();
	...
	private:
		...
		struct ClientRef {
			...
			std::conditional_t<is_owner, std::unique_ptr<UdpClient>, UdpClient*> client = nullptr;
			IpAddress ip;
			NetworkSocket::port_type port = 0;

			ClientRef() = default;
			ClientRef(decltype(client)&& client, IpAddress, NetworkSocket::port_type port = 0)
			...
			bool operator==(const ClientRef<Owner2>& p_other) const {
		...
		std::vector<ClientRef<false>> _clients;
