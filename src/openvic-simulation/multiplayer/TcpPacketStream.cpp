...
NetworkError TcpPacketStream::poll() {
	...
	case CONNECTING: return NetworkError::OK;
	default:		 break;
	}
...
NetworkError TcpPacketStream::set_data(std::span<uint8_t> buffer) {
	size_t _ = 0;
	return _set_data<true>(buffer, _);
}

NetworkError TcpPacketStream::set_data_no_blocking(std::span<uint8_t> buffer, size_t& r_sent) {
	return _set_data<false>(buffer, r_sent);
}

NetworkError TcpPacketStream::get_data(std::span<uint8_t> buffer) {
	return _get_data<true>(buffer);
}

NetworkError TcpPacketStream::get_data_no_blocking(std::span<uint8_t> buffer, size_t& r_received) {
	NetworkError result = _get_data<false>(buffer);
	r_received = buffer.size();
	return result;
}

template<bool IsBlocking>
NetworkError TcpPacketStream::_set_data(std::span<uint8_t> buffer, size_t& r_sent) {
	OV_ERR_FAIL_COND_V(_socket == nullptr, NetworkError::UNAVAILABLE);
...
template NetworkError TcpPacketStream::_set_data<true>(std::span<uint8_t> buffer, size_t& r_sent);
template NetworkError TcpPacketStream::_set_data<false>(std::span<uint8_t> buffer, size_t& r_sent);
