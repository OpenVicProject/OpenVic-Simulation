...
static std::string _get_cache_key(std::string_view p_hostname, NetworkResolverBase::Type p_type) {
	std::string key { p_hostname };
...
void NetworkResolverBase::ResolveHandler::resolve_queues() {
	for (int i = 0; i < MAX_QUERIES; i++) {
		...
		std::vector<IpAddress> response = NetworkResolver::singleton()._resolve_hostname(hostname, type);
		...
		if (response.size()) {
			std::string key = _get_cache_key(hostname, type);
			cache[key] = response;
		}
...
IpAddress NetworkResolverBase::resolve_hostname(std::string_view p_hostname, Type p_type) {
	const std::vector<IpAddress> addresses = resolve_hostname_addresses(p_hostname, p_type);
	return addresses.empty() ? IpAddress {} : addresses.front();
}

std::vector<IpAddress> NetworkResolverBase::resolve_hostname_addresses(std::string_view p_hostname, Type p_type) {
	std::string key = _get_cache_key(p_hostname, p_type);
	{
		...
		if (_resolve_handler.cache.contains(key)) {
			...
			// other requests.
			lock.unlock();
			std::vector<IpAddress> result = _resolve_hostname(p_hostname, p_type);
			lock.lock();
			// We might be overriding another result, but we don't care as long as the result is valid.
			if (result.size()) {
...
int32_t NetworkResolverBase::resolve_hostname_queue_item(std::string_view p_hostname, Type p_type) {
	...
	if (_resolve_handler.cache.contains(key)) {
		...
		_resolve_handler.queue[id].response = std::vector<IpAddress>();
...
std::vector<IpAddress> NetworkResolverBase::get_resolve_item_addresses(int32_t p_id) const {
	OV_ERR_FAIL_INDEX_V_MSG(
		p_id, MAX_QUERIES, std::vector<IpAddress> {},
		fmt::format(
			"Too many concurrent DNS resolver queries ({}, but should be {} at most). Try performing less network requests at "
			"once.",
			p_id, MAX_QUERIES
	...
	return _resolve_handler.queue[p_id].response | ranges::views::filter([](IpAddress const& addr) -> bool {
		...
		ranges::to<std::vector<IpAddress>>();
}
...
std::vector<IpAddress> NetworkResolverBase::get_local_addresses() const {
	...
	for (std::pair<std::string, InterfaceInfo> const& pair : interfaces | ranges::views::reverse) {
