...
namespace OpenVic {
	...
	struct BasicStreamPacketClient : PacketClient {
		BasicStreamPacketClient(uint8_t max_buffer_power = 16) {
			_ring_buffer.reserve(1 << max_buffer_power);
		...
		int64_t available_packets() const override {
			...
			while (remaining >= 4) {
				...
				uint32_t length = 0;
				zpp::bits::in in(lbuf);
				OV_ERR_BREAK(in(length) != std::errc {});
				remaining -= 4;
				offset += 4;
				if (length > remaining) {
					break;
		...
		void set_input_buffer_max_size(int p_max_size) {
			...
			_ring_buffer.reserve(1 << (std::bit_width<uint8_t>(std::bit_ceil<uint64_t>(p_max_size + 4)) - 1));
	...
	private:
		NetworkError _set_packet(std::span<uint8_t> buffer) override {
			...
			zpp::bits::out out(_output_buffer);
			OV_ERR_FAIL_COND_V(failure(out(buffer)), NetworkError::FAILED);

			return _stream->set_data(_output_buffer);
		}
		...
		NetworkError _get_packet(std::span<uint8_t>& r_set) override {
			...
			uint32_t length;
			zpp::bits::in in(lbuf);
			OV_ERR_FAIL_COND_V(in(length) != std::errc {}, NetworkError::UNAVAILABLE);
			...
			_ring_buffer.erase(_ring_buffer.begin(), sizeof(length));
