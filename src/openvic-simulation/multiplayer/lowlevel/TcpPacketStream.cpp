...
NetworkError TcpPacketStream::get_data(std::span<uint8_t> buffer_to_set) {
	return _get_data<get_data_behavior::no_wait>(buffer_to_set);
}

NetworkError TcpPacketStream::get_data_no_blocking(std::span<uint8_t> buffer_to_set, size_t& r_received) {
	NetworkError result = _get_data<get_data_behavior::no_block>(buffer_to_set);
	r_received = buffer_to_set.size();
	return result;
}

NetworkError TcpPacketStream::get_data_waiting(std::span<uint8_t> buffer_to_set) {
	return _get_data<get_data_behavior::wait>(buffer_to_set);
}
...
template<TcpPacketStream::get_data_behavior Behavior>
NetworkError TcpPacketStream::_get_data(std::span<uint8_t>& r_buffer) {
	if (_status != Status::CONNECTED) {
		r_buffer = { r_buffer.data(), 0 };
		return NetworkError::FAILED;
	}
	...
	for (size_t iteration = 0, last_it_check = iteration + 1; to_read; ++iteration) {
		int64_t read = 0;
		...
		if (err != NetworkError::OK) {
			...
			if constexpr (Behavior == get_data_behavior::no_wait) {
#if defined(__APPLE__) && defined(__MACH__)
				if (iteration < last_it_check) {
					// Mac does not support avoiding polling waits
					err = _socket->poll(NetworkSocket::PollType::IN, -1);
					last_it_check = iteration + 1;
				}
			...
			if constexpr (Behavior == get_data_behavior::no_block) {
				r_buffer = { r_buffer.data(), static_cast<size_t>(total_read) };
				return NetworkError::OK;
			} else {
				err = _socket->poll(NetworkSocket::PollType::IN, -1);
...
template NetworkError TcpPacketStream::_get_data<TcpPacketStream::get_data_behavior::wait>(std::span<uint8_t>& r_buffer);
template NetworkError TcpPacketStream::_get_data<TcpPacketStream::get_data_behavior::no_wait>(std::span<uint8_t>& r_buffer);
template NetworkError TcpPacketStream::_get_data<TcpPacketStream::get_data_behavior::no_block>(std::span<uint8_t>& r_buffer);
