...
namespace OpenVic {
	struct NetworkResolverBase {
		...
		std::vector<IpAddress> resolve_hostname_addresses(std::string_view p_hostname, Type p_type = Type::ANY);
		...
		std::vector<IpAddress> get_resolve_item_addresses(int32_t p_id) const;
		...
		std::vector<IpAddress> get_local_addresses() const;
		virtual OpenVic::string_map_t<InterfaceInfo> get_local_interfaces() const = 0;

		virtual Provider provider() const = 0;

	protected:
		virtual std::vector<IpAddress> _resolve_hostname(std::string_view p_hostname, Type p_type = Type::ANY) const = 0;
		...
		struct ResolveHandler {
			...
			string_map_t<std::vector<IpAddress>> cache;
			std::mutex mutex;
			std::thread thread;
