...
namespace OpenVic {
	...
	private:
		...
		enum class get_data_behavior {
			wait, // Blocks and waits for data to fill buffer
			no_wait, // Blocks but will not wait for data to fill buffer
			no_block // Neither blocks nor waits
		};

		template<bool IsBlocking>
		NetworkError _set_data(std::span<const uint8_t> buffer, size_t& r_sent);

		template<get_data_behavior Behavior>
	...
	extern template NetworkError TcpPacketStream::_get_data<TcpPacketStream::get_data_behavior::wait>( //
		std::span<uint8_t>& r_buffer
	);
	extern template NetworkError TcpPacketStream::_get_data<TcpPacketStream::get_data_behavior::no_wait>( //
		std::span<uint8_t>& r_buffer
	);
	extern template NetworkError TcpPacketStream::_get_data<TcpPacketStream::get_data_behavior::no_block>( //
		std::span<uint8_t>& r_buffer
	);
}
