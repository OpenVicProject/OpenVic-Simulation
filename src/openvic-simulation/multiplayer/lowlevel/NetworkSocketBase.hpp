...
namespace OpenVic {
	struct NetworkSocketBase {
		...
		std::unique_ptr<NetworkSocketBase> accept(IpAddress& r_ip, port_type& r_port) {
			return accept_as<NetworkSocketBase>(r_ip, r_port);
		}

		template<std::derived_from<NetworkSocketBase> T>
		std::unique_ptr<T> accept_as(IpAddress& r_ip, port_type& r_port) {
			return std::unique_ptr<T> { static_cast<T*>(_accept(r_ip, r_port)) };
		}
