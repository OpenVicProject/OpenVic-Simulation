...
UdpClient::UdpClient() : _socket(std::make_shared<NetworkSocket>()), _ring_buffer(1 << 16) {} // NOLINT
...
NetworkError UdpClient::bind(NetworkSocket::port_type port, IpAddress const& address, size_t min_buffer_size) {
	...
	_ring_buffer.reserve(1 << std::bit_ceil(min_buffer_size));
