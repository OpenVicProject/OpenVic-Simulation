...
#include "openvic-simulation/multiplayer/UdpServer.hpp"

using namespace OpenVic;

UdpClient::UdpClient() : _ring_buffer(16) {} // NOLINT
...
void UdpClient::close() {
	if (_server) {
		_server->remove_client(_peer_ip, _peer_port);
		_server = nullptr;
		_socket = std::make_unique<NetworkSocket>();
	} else if (_socket) {
		_socket->close();
	}
...
NetworkError UdpClient::set_destination(HostnameAddress const& addr, NetworkSocket::port_type port) {
	...
	_peer_ip = addr.resolved_address();
	_peer_port = port;
	return NetworkError::OK;
}
...
NetworkError UdpClient::_set_packet(std::span<uint8_t> buffer) {
	OV_ERR_FAIL_COND_V(_socket == nullptr, NetworkError::UNAVAILABLE);
	OV_ERR_FAIL_COND_V(!_peer_ip.is_valid(), NetworkError::UNCONFIGURED);

	NetworkError err;
	int64_t sent = -1;

	if (!_socket->is_open()) {
		NetworkResolver::Type ip_type = _peer_ip.is_ipv4() ? NetworkResolver::Type::IPV4 : NetworkResolver::Type::IPV6;
		err = _socket->open(NetworkSocket::Type::UDP, ip_type);
		OV_ERR_FAIL_COND_V(err != NetworkError::OK, err);
		_socket->set_blocking_enabled(false);
		_socket->set_broadcasting_enabled(_is_broadcasting);
	...
	do {
		if (_is_connected && !_server) {
			...
			err = _socket->send_to(buffer.data(), buffer.size(), sent, _peer_ip, _peer_port);
		}
		if (err != NetworkError::OK) {
