...
bool PacketTypes::retransmit_packet_send_callback(
	...
	RetransmitData const* const* data_ptr = std::get_if<RetransmitData const*>(&argument);
...
bool PacketTypes::broadcast_packet_send_callback(
	...
	BroadcastData const* const* data_ptr = std::get_if<BroadcastData const*>(&argument);
	OV_ERR_FAIL_NULL_V(data_ptr, false);
...
bool PacketTypes::send_raw_packet_send_callback(
	...
	std::span<uint8_t> const* data = std::get_if<std::span<uint8_t>>(&argument);
...
bool PacketTypes::update_host_session_send_callback(
	...
	HostSession const* const* session_ptr = std::get_if<HostSession const*>(&argument);
...
bool PacketTypes::execute_game_action_send_callback(
	...
	game_action_t const* const* action_ptr = std::get_if<game_action_t const*>(&argument);
...
bool PacketTypes::send_chat_message_send_callback(
	...
	ChatMessageLog const* const* message_ptr = std::get_if<ChatMessageLog const*>(&argument);
...
bool PacketTypes::send_chat_message_process_callback(BaseMultiplayerManager* multiplayer_manager, PacketSpan packet) {
	...
	BaseMultiplayerManager::client_id_type from_id = packet.read<decltype(from_id)>();
...
bool PacketTypes::add_chat_group_send_callback(
	...
	std::span<uint64_t> const* clients_ptr = std::get_if<std::span<uint64_t>>(&argument);
...
bool PacketTypes::modify_chat_group_send_callback(
	...
	PacketChatGroupModifyData const* modify_data_ptr = std::get_if<PacketChatGroupModifyData>(&argument);
...
bool PacketTypes::delete_chat_group_send_callback(
	...
	ChatGroup::index_t const* group_id_ptr = std::get_if<ChatGroup::index_t>(&argument);
