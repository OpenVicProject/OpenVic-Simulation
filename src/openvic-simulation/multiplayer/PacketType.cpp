...
bool PacketTypes::send_raw_packet_send_callback(
	...
	memory::vector<uint8_t> const* data = std::any_cast<memory::vector<uint8_t>>(&argument);
	if (data == nullptr) {
...
bool PacketTypes::send_chat_message_send_callback(
	...
	ChatManager::MessageData const* const* message_ptr = std::any_cast<ChatManager::MessageData const*>(&argument);
	if (!message_ptr) {
		message_ptr = std::any_cast<ChatManager::MessageData*>(&argument);
	}
	OV_ERR_FAIL_NULL_V(message_ptr, false);

	ChatManager::MessageData const& message = **message_ptr;

	ClientManager const* client = multiplayer_manager->cast_as<ClientManager>();
	if (client != nullptr) {
		builder.put_back(client->get_client_id());
	} else {
		builder.put_back(HostManager::HOST_ID);
	}
...
bool PacketTypes::send_chat_message_process_callback(BaseMultiplayerManager* multiplayer_manager, PacketSpan packet) {
	...
	ChatManager::MessageData message = packet.read<decltype(message)>();

	game_manager->get_chat_manager().log_message(from_id, std::move(message));
