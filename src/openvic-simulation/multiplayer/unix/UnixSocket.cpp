...
UnixSocket UnixSocket::accept(IpAddress& r_ip, port_type& r_port) {
...
std::unique_ptr<NetworkSocketBase> UnixSocket::virtual_accept(IpAddress& r_ip, port_type& r_port) {
	UnixSocket socket = accept(r_ip, r_port);

	if (!socket.is_open()) {
		return nullptr;
	}

	return std::make_unique<UnixSocket>(std::move(socket));
}
