...
namespace OpenVic {
	struct TcpPacketStream : PacketStream {
		...
		NetworkError get_data(std::span<uint8_t> buffer) override;
		NetworkError get_data_no_blocking(std::span<uint8_t> buffer, size_t& r_received) override;
	...
	private:
		...
		NetworkError _set_data(std::span<uint8_t> buffer, size_t& r_sent);

		template<bool IsBlocking>
		NetworkError _get_data(std::span<uint8_t>& r_buffer);
	};

	extern template NetworkError TcpPacketStream::_set_data<true>(std::span<uint8_t> buffer, size_t& r_sent);
	extern template NetworkError TcpPacketStream::_set_data<false>(std::span<uint8_t> buffer, size_t& r_sent);
