...
bool Element::_fill_key_map(NodeTools::case_insensitive_key_map_t& key_map, UIManager const& ui_manager) {
	...
	static const string_map_t<orientation_t> orientation_map = {
		{ "UPPER_LEFT", UPPER_LEFT },   { "LOWER_LEFT", LOWER_LEFT }, { "LOWER_RIGHT", LOWER_RIGHT },
		{ "UPPER_RIGHT", UPPER_RIGHT }, { "CENTER", CENTER },         { "CENTER_UP", CENTER_UP },
	...
	ret &= add_key_map_entries(
	    ...
	    expect_identifier_or_string(expect_mapped_string(
	        orientation_map,
	        assign_variable_callback(orientation),
...
bool Element::_fill_elements_key_map(
    ...
    callback_t<memory::unique_base_ptr<Element>&&> callback,
	...
	ret &= add_key_map_entries(
	    ...
	    "iconType",
	    ZERO_OR_MORE,
	    _expect_instance<Element, Icon>(callback, ui_manager),
	    "shieldtype",
	    ZERO_OR_MORE,
	    _expect_instance<Element, Icon>(callback, ui_manager),
	    "guiButtonType",
	    ZERO_OR_MORE,
	    _expect_instance<Element, Button>(callback, ui_manager),
	    "checkboxType",
	    ZERO_OR_MORE,
	    _expect_instance<Element, Checkbox>(callback, ui_manager),
	    "textBoxType",
	    ZERO_OR_MORE,
	    _expect_instance<Element, Text>(callback, ui_manager),
	    "instantTextBoxType",
	    ZERO_OR_MORE,
	    _expect_instance<Element, Text>(callback, ui_manager),
	    "OverlappingElementsBoxType",
	    ZERO_OR_MORE,
	    _expect_instance<Element, OverlappingElementsBox>(callback, ui_manager),
	    "listboxType",
	    ZERO_OR_MORE,
	    _expect_instance<Element, ListBox>(callback, ui_manager),
	    "editBoxType",
	    ZERO_OR_MORE,
	    _expect_instance<Element, TextEditBox>(callback, ui_manager),
	    "scrollbarType",
	    ZERO_OR_MORE,
...
bool Window::_fill_key_map(NodeTools::case_insensitive_key_map_t& key_map, UIManager const& ui_manager) {
	...
	ret &= add_key_map_entries(
	    ...
	    "size",
	    ONE_EXACTLY,
	    expect_fvec2(assign_variable_callback(size)),
	    "moveable",
	    ZERO_OR_ONE,
	    expect_int_bool(assign_variable_callback(moveable)),
	    "fullScreen",
	    ZERO_OR_ONE,
	    expect_bool(assign_variable_callback(fullscreen)),
	    "backGround",
	    ZERO_OR_ONE,
	    ...
	    "dontRender",
	    ZERO_OR_ONE,
	    success_callback, // always empty string?
	    "horizontalBorder",
	    ZERO_OR_ONE,
	    success_callback,
	    "verticalBorder",
...
bool Icon::_fill_key_map(NodeTools::case_insensitive_key_map_t& key_map, UIManager const& ui_manager) {
	...
	ret &= add_key_map_entries(
	    ...
	    "frame",
	    ZERO_OR_ONE,
	    expect_uint(assign_variable_callback(frame)),
	    "scale",
	    ZERO_OR_ONE,
...
bool BaseButton::_fill_key_map(NodeTools::case_insensitive_key_map_t& key_map, UIManager const& ui_manager) {
	...
	ret &= add_key_map_entries(
	    ...
	    "buttonText",
	    ZERO_OR_ONE,
	    expect_string(assign_variable_callback_string(text), true),
	    /* Some buttons have multiple fonts listed with the last one being used. */
	    "buttonFont",
	    ZERO_OR_MORE,
	    ui_manager.expect_font_string(assign_variable_callback_pointer(font), true),
	    "shortcut",
	    ZERO_OR_ONE,
	    expect_string(assign_variable_callback_string(shortcut), true),
...
bool Button::_fill_key_map(NodeTools::case_insensitive_key_map_t& key_map, UIManager const& ui_manager) {
	...
	ret &= add_key_map_entries(
	    ...
	    "format",
	    ZERO_OR_ONE,
	    success_callback, /* Is always left from what I've seen. */
	    "clicksound",
	    ZERO_OR_ONE,
	    ...
	    "parent",
	    ZERO_OR_ONE,
	    success_callback /* Links buttons to a scrollbar, not needed thanks to contextual info. */
	);
	return ret;
}
...
bool Text::_fill_key_map(NodeTools::case_insensitive_key_map_t& key_map, UIManager const& ui_manager) {
	...
	ret &= add_key_map_entries(
	    ...
	    "font",
	    ONE_EXACTLY,
	    ui_manager.expect_font_string(assign_variable_callback_pointer(font)),
	    "maxWidth",
	    ZERO_OR_ONE,
	    expect_fixed_point(assign_variable_callback(max_size.x)),
	    "maxHeight",
	    ZERO_OR_ONE,
...
bool ListBox::_fill_key_map(NodeTools::case_insensitive_key_map_t& key_map, UIManager const& ui_manager) {
	...
	ret &= add_key_map_entries(
	    ...
	    "offset",
	    ZERO_OR_ONE,
	    expect_fvec2(assign_variable_callback(scrollbar_offset)),
	    "borderSize",
	    ZERO_OR_ONE,
	    expect_fvec2(assign_variable_callback(items_offset)),
	    "spacing",
	    ZERO_OR_ONE,
	    expect_fixed_point(assign_variable_callback(spacing)),
	    "scrollbartype",
...
bool Scrollbar::_fill_key_map(NodeTools::case_insensitive_key_map_t& key_map, UIManager const& ui_manager) {
	...
	ret &= add_key_map_entries(
	    ...
	    "slider",
	    ONE_EXACTLY,
	    expect_string(assign_variable_callback_string(slider_button_name)),
	    "track",
	    ONE_EXACTLY,
	    expect_string(assign_variable_callback_string(track_button_name)),
	    "leftbutton",
	    ONE_EXACTLY,
	    expect_string(assign_variable_callback_string(less_button_name)),
	    "rightbutton",
	    ONE_EXACTLY,
	    expect_string(assign_variable_callback_string(more_button_name)),
	    "size",
	    ONE_EXACTLY,
	    expect_fvec2(assign_variable_callback(size)),
	    "borderSize",
	    ...
	    "minValue",
	    ONE_EXACTLY,
	    expect_fixed_point(assign_variable_callback(min_value)),
	    "maxValue",
	    ONE_EXACTLY,
	    expect_fixed_point(assign_variable_callback(max_value)),
	    "stepSize",
	    ONE_EXACTLY,
	    expect_fixed_point(assign_variable_callback(step_size)),
	    "startValue",
	    ONE_EXACTLY,
	    expect_fixed_point(assign_variable_callback(start_value)),
	    "horizontal",
	    ONE_EXACTLY,
	    expect_int_bool(assign_variable_callback(horizontal)),
	    "useRangeLimit",
	    ZERO_OR_ONE,
	    expect_bool(assign_variable_callback(range_limited)),
	    "rangeLimitMin",
	    ZERO_OR_ONE,
	    expect_fixed_point(assign_variable_callback(range_limit_min)),
	    "rangeLimitMax",
	    ZERO_OR_ONE,
	    expect_fixed_point(assign_variable_callback(range_limit_max)),
	    "rangeLimitMinIcon",
	    ZERO_OR_ONE,
	    expect_string(assign_variable_callback_string(range_limit_min_icon_name)),
	    "rangeLimitMaxIcon",
	    ZERO_OR_ONE,
	    ...
	    "guiButtonType",
	    ONE_OR_MORE,
	    _expect_instance<Element, Button>(add_element, ui_manager),
	    "iconType",
	    ZERO_OR_MORE,
