...
node_callback_t Sprite::expect_sprites(
    length_callback_t length_callback, callback_t<memory::unique_base_ptr<Sprite>&&> callback
) {
	return expect_dictionary_keys_and_length(
	    length_callback,
	    "spriteType",
	    ZERO_OR_MORE,
	    _expect_instance<Sprite, IconTextureSprite>(callback),
	    "progressbartype",
	    ZERO_OR_MORE,
	    _expect_instance<Sprite, ProgressBar>(callback),
	    "PieChartType",
	    ZERO_OR_MORE,
	    _expect_instance<Sprite, PieChart>(callback),
	    "LineChartType",
	    ZERO_OR_MORE,
	    _expect_instance<Sprite, LineChart>(callback),
	    "textSpriteType",
	    ZERO_OR_MORE,
	    _expect_instance<Sprite, IconTextureSprite>(callback),
	    "maskedShieldType",
	    ZERO_OR_MORE,
	    _expect_instance<Sprite, MaskedFlag>(callback),
	    "tileSpriteType",
	    ZERO_OR_MORE,
	    _expect_instance<Sprite, TileTextureSprite>(callback),
...
bool IconTextureSprite::_fill_key_map(case_insensitive_key_map_t& key_map) {
	...
	ret &= add_key_map_entries(
	    ...
	    "noOfFrames",
	    ZERO_OR_ONE,
	    expect_uint(assign_variable_callback(no_of_frames)),
...
bool ProgressBar::_fill_key_map(case_insensitive_key_map_t& key_map) {
	...
	ret &= add_key_map_entries(
	    ...
	    "textureFile1",
	    ZERO_OR_ONE,
	    expect_string(assign_variable_callback_string(progress_texture_file)),
	    "textureFile2",
	    ZERO_OR_ONE,
	    expect_string(assign_variable_callback_string(back_texture_file)),
	    "size",
	    ONE_EXACTLY,
	    expect_ivec2(assign_variable_callback(size)),
...
bool MaskedFlag::_fill_key_map(case_insensitive_key_map_t& key_map) {
	...
	ret &= add_key_map_entries(
	    ...
	    "textureFile1",
	    ONE_EXACTLY,
	    expect_string(assign_variable_callback_string(overlay_file)),
	    "textureFile2",
	    ONE_EXACTLY,
	    expect_string(assign_variable_callback_string(mask_file)),
