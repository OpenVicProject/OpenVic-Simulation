...
namespace OpenVic::utility {
	...
	template<typename T>
	concept Encodable = utility::specialization_of<T, std::variant> || std::integral<T> || std::floating_point<T> ||
		...
		std::convertible_to<T, std::span<const uint8_t>> || utility::specialization_of<T, std::pair> ||
		utility::specialization_of<T, std::tuple> || utility::specialization_of<T, vec2_t> ||
		utility::specialization_of<T, vec3_t> || utility::specialization_of<T, vec4_t> ||
	...
	static inline size_t encode(T const& value, std::span<uint8_t> span = {}) {
	...
	template<typename T>
	concept Decodable = utility::specialization_of<T, std::variant> || std::integral<T> || std::floating_point<T> ||
		std::is_enum_v<T> || std::convertible_to<std::string_view, T> || std::is_constructible_v<T, std::string_view> ||
		std::same_as<T, half_float> || std::same_as<T, Timespan> || std::same_as<T, Date> || IsColour<T> ||
		std::same_as<T, fixed_point_t> || std::is_constructible_v<T, uint8_t*, uint8_t*> ||
		utility::specialization_of<T, std::pair> || utility::specialization_of<T, std::tuple> ||
		utility::specialization_of_std_array_of<T, uint8_t> || utility::specialization_of<T, vec2_t> ||
		utility::specialization_of<T, vec3_t> || utility::specialization_of<T, vec4_t> ||
		(std::is_empty_v<T> && std::is_default_constructible_v<T>);
