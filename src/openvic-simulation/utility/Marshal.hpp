...
namespace OpenVic::utility {
	...
	template<typename T>
	concept Encodable = HasEncode<T> || utility::specialization_of<T, std::variant> || std::integral<T> ||
		std::floating_point<T> || std::is_enum_v<T> || std::convertible_to<T, std::string_view> ||
		std::same_as<T, half_float> || std::same_as<T, Timespan> || std::same_as<T, Date> || IsColour<T> ||
		std::same_as<T, fixed_point_t> || std::convertible_to<T, std::span<const uint8_t>> ||
		utility::specialization_of<T, std::vector> || utility::specialization_of<T, OpenVic::utility::deque> ||
	...
	static inline size_t encode( //
		...
		if constexpr (std::is_const_v<T> || std::is_volatile_v<T> || std::is_reference_v<T>) {
			...
			const size_t size = collection_size + length_size;
			...
			for (auto const& element : value) {
				offset += encode(element, span.subspan(offset));
			}
	...
	static inline T decode( //
		...
		if constexpr (std::is_const_v<T> || std::is_volatile_v<T>) {
			...
			for (size_t index = 0; index < size; index++) {
				value.emplace_back(decode<T::value_type>(span.subspan(offset), r_decode_count));
