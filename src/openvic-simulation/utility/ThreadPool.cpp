...
void ThreadPool::loop_until_cancelled(
	work_t& work_type, GameRulesManager const& game_rules_manager, GoodInstanceManager const& good_instance_manager,
	ModifierEffectCache const& modifier_effect_cache, PopsDefines const& pop_defines,
	ProductionTypeManager const& production_type_manager, forwardable_span<const CountryInstance> country_keys,
	forwardable_span<const GoodDefinition> good_keys, forwardable_span<const Strata> strata_keys,
	...
	static constexpr size_t VECTOR_COUNT = std::max(
		...
		std::max(CountryInstance::VECTORS_FOR_COUNTRY_TICK, ProvinceInstance::VECTORS_FOR_PROVINCE_TICK)
	);
	...
	PopValuesFromProvince reusable_pop_values { game_rules_manager,		 good_instance_manager, modifier_effect_cache,
												production_type_manager, pop_defines,			strata_keys };

	while (!is_cancellation_requested) {
		work_t work_type_copy;
		{
			std::unique_lock<std::mutex> thread_lock { thread_mutex };
			thread_condition.wait(thread_lock, [this, &work_type]() -> bool {
			...
			for (WorkBundle& work_bundle : work_bundles) {
				for (ProvinceInstance& province : work_bundle.provinces_chunk) {
					province.province_tick(
						current_date, reusable_pop_values, work_bundle.random_number_generator, reusable_goods_mask,
			...
			for (WorkBundle& work_bundle : work_bundles) {
				for (CountryInstance& country : work_bundle.countries_chunk) {
					country.country_tick_before_map(
						reusable_goods_mask, reusable_vectors_span.first<CountryInstance::VECTORS_FOR_COUNTRY_TICK>(),
...
void ThreadPool::await_completion() {
	...
	completed_condition.wait(completed_lock, [this]() -> bool {
		return active_work_count == 0;
	});
}

ThreadPool::ThreadPool(Date const& new_current_date) : current_date { new_current_date } {}
...
void ThreadPool::initialise_threadpool(
	GameRulesManager const& game_rules_manager, GoodInstanceManager const& good_instance_manager,
	ModifierEffectCache const& modifier_effect_cache, PopsDefines const& pop_defines,
	ProductionTypeManager const& production_type_manager, forwardable_span<const GoodDefinition> good_keys,
	forwardable_span<const Strata> strata_keys, forwardable_span<GoodInstance> goods,
	forwardable_span<CountryInstance> countries, forwardable_span<ProvinceInstance> provinces
) {
	if (threads.size() > 0) {
	...
	for (size_t i = 0; i < WORK_BUNDLE_COUNT; i++) {
		const size_t goods_chunk_size = i < goods_remainder ? goods_quotient + 1 : goods_quotient;
		const size_t countries_chunk_size = i < countries_remainder ? countries_quotient + 1 : countries_quotient;
		const size_t provinces_chunk_size = i < provinces_remainder ? provinces_quotient + 1 : provinces_quotient;
		...
		all_work_bundles[i] =
			WorkBundle { master_rng.generator().serialize(), std::span<CountryInstance> { countries_begin, countries_end },
	...
	for (size_t i = 0; i < max_worker_threads; ++i) {
		...
		threads.emplace_back(
			[this, &work_for_thread = work_per_thread[i], &game_rules_manager, &good_instance_manager, &modifier_effect_cache,
			 &pop_defines, &production_type_manager, countries, good_keys, strata_keys, work_bundles_begin,
			 work_bundles_end]() -> void {
				loop_until_cancelled(
					work_for_thread, game_rules_manager, good_instance_manager, modifier_effect_cache, pop_defines,
					production_type_manager, countries, good_keys, strata_keys,
