...
void ThreadPool::loop_until_cancelled(
	...
	memory::FixedVector<fixed_point_t, country_index_t> reusable_country_map_0 {
		country_index_t(country_keys.size()), fixed_point_t::_0
	};
	...
	PopValuesFromProvince reusable_pop_values {
		game_rules_manager, good_instance_manager, modifier_effect_cache, production_type_manager, pop_defines, strata_count
	};
...
void ThreadPool::initialise_threadpool(
	...
	for (std::size_t i = 0; i < WORK_BUNDLE_COUNT; i++) {
		const std::size_t goods_chunk_size = i < goods_remainder ? goods_quotient + 1 : goods_quotient;
		const std::size_t countries_chunk_size = i < countries_remainder ? countries_quotient + 1 : countries_quotient;
		const std::size_t provinces_chunk_size = i < provinces_remainder ? provinces_quotient + 1 : provinces_quotient;
	...
	const std::size_t max_worker_threads = std::min(
	    std::max<std::size_t>(std::thread::hardware_concurrency(), 1), WORK_BUNDLE_COUNT
	);
	...
	for (std::size_t i = 0; i < max_worker_threads; ++i) {
		const std::size_t work_bundles_chunk_size =
		    i < work_bundles_remainder ? work_bundles_quotient + 1 : work_bundles_quotient;
