...
namespace OpenVic::memory {
	...
	template<typename T, class RawAllocator, typename... Args>
	requires(!std::is_array_v<T>)
	static inline T* allocate_new(allocator_reference<RawAllocator> alloc, Args&&... args) {
		T* ptr = alloc.try_allocate_node(sizeof(T), alignof(T));
		if (!ptr) {
			return ptr;
		}
	...
	static inline T* allocate_new(allocator_reference<RawAllocator> alloc, size_t n) {
		return alloc.try_allocate_node(sizeof(T) * n, alignof(T));
	}
	...
	template<typename T, class RawAllocator>
	requires(!std::is_array_v<T>)
	static inline void dealloc_delete(allocator_reference<RawAllocator> alloc, T* ptr) {
		ptr->~T();
	...
	template<typename T, class RawAllocator>
	requires(std::is_unbounded_array_v<T>)
	static inline void dealloc_delete(allocator_reference<RawAllocator> alloc, T* ptr, size_t n) {
		std::destroy_n(ptr, n);
		alloc.deallocate(ptr, sizeof(T) * n, alignof(T));
	}
