		...
		return [&entry, add](Culture const& culture) -> bool {
			...
			if (it == entry.accepted_cultures.end()) {
				...
				spdlog::warn_s(
					...
					add ? "add" : "remove", add ? "to" : "from", culture.get_identifier(), entry.get_country()
				);
				return true;
			} else {
				...
				return issue_manager.expect_reform_identifier([&entry, reform_group](Reform const& reform) -> bool {
					if (&reform.get_reform_group() != reform_group) {
						spdlog::warn_s(
							...
							reform.get_identifier(), *reform_group, reform.get_reform_group(), entry.get_country()
						);
					}
						...
						[&entry, technology](decltype(entry.technologies)::mapped_type value) -> bool {
							if (value > 1) {
								spdlog::warn_s(
									...
									technology->get_identifier(), entry.get_country().get_identifier()
								);
							}
					...
					if (id == "government") {
						...
						if (flag_expected) {
							spdlog::error_s(
								...
								fmt::validate(government_type), entry.get_country()
							);
							ret = false;
						}
						flag_expected = true;
