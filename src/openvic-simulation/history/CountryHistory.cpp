...
void CountryHistoryManager::lock_country_histories() {
	for (const auto& entry : country_histories) {
		if (entry.second.size() == 0) {
			Logger::error("Attempted to lock country histories - country ",
				entry.first->get_identifier(), " has no history entries!");
		}
	}
...
inline bool CountryHistoryManager::_load_country_history_entry(GameManager& game_manager,
	std::string_view name, Date const& date, ast::NodeCPtr root) {
	...
	bool ret = expect_dictionary_keys_and_default(
		...
		"nationalvalue", ZERO_OR_ONE, game_manager.get_politics_manager().get_national_value_manager()
			.expect_national_value_identifier(assign_variable_callback_pointer(national_value)),
		...
		"ruling_party", ZERO_OR_ONE, expect_identifier([this, &game_manager,
			&ruling_party, &name, &date](std::string_view identifier) -> bool {
		...
		"oob", ZERO_OR_ONE, [&game_manager, &initial_oob](ast::NodeCPtr node) -> bool {
			...
			if (string.starts_with('/')) {
				...
				if (game_manager.get_military_manager().get_deployment_manager().has_deployment_identifier(string)) {
					initial_oob = game_manager.get_military_manager().get_deployment_manager()
						.get_deployment_by_identifier(string);
				}
			}
	...
	ret &= add_country_history_entry(game_manager.get_country_manager().get_country_by_identifier(name), date,
		primary_culture, std::move(accepted_cultures), religion, ruling_party, last_election, std::move(upper_house),
		capital, government_type, plurality, national_value, civilised, prestige, std::move(reforms), initial_oob,
		updated_accepted_cultures, updated_upper_house, updated_reforms);
	return ret;
}
