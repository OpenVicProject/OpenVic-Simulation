...
CountryInstanceManager::CountryInstanceManager(
    ...
    shared_country_values { new_pop_defines, new_good_instance_manager, pop_type_keys, regiment_types }, country_instances {
	    country_index_t(new_country_definition_manager.get_country_definition_count()),
	    [this, &new_country_definition_manager, &country_instance_deps](const country_index_t country_index) -> auto {
...
void CountryInstanceManager::update_rankings(const Date today) {
	...
	std::stable_sort(
	    industrial_power_ranking.begin(),
	    industrial_power_ranking.end(),
...
CountryInstance const* CountryInstanceManager::get_country_instance_by_identifier(std::string_view identifier) const {
	CountryDefinition const* country_definition = country_definition_manager.get_country_definition_by_identifier(identifier);
	return country_definition == nullptr ? nullptr : &(country_instances[country_definition->index]);
}
...
CountryInstance const& CountryInstanceManager::get_country_instance_by_definition(
