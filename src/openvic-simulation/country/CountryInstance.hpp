...
namespace OpenVic {
	...
	private:
		...
		memory::vector<std::pair<std::reference_wrapper<const State>, fixed_point_t>> SPAN_PROPERTY(
		    industrial_power_from_states
		);
		memory::vector<std::pair<std::reference_wrapper<const CountryInstance>, fixed_point_t>> SPAN_PROPERTY(
	...
	private:
		...
		constexpr bool validate_unit_unlock_change(
			...
			if (unlock_level + unlock_level_change < 0) {
				spdlog::error_s(
				    "Attempted to change unlock level for {} in country {} to invalid value: current level = {}, change = {}, "
				    "invalid new value = {}",
				    index,
				    *this,
				    unlock_level,
