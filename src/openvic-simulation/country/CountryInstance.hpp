...
namespace OpenVic {
	...
	private:
		...
		memory::unique_ptr<std::mutex> import_value_mutex;
	...
	public:
		...
		// These functions take "memory::string const&" rather than "std::string_view" as they're only used with script arguments
		// which are always stored as "memory::string"s and it significantly simplifies mutable value access.
	...
	public:
		CountryInstanceManager(
			...
			memory::vector<PopType> const& pop_type_keys
		);
