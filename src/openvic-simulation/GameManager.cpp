...
bool GameManager::load_mods(Dataloader::path_vector_t& roots, Dataloader::path_vector_t& replace_paths, std::span<std::string> requested_mods) {
	bool ret = true;
	...
	for (std::string_view requested_mod : requested_mods) {
		...
		if(!_get_mod_dependencies(&*it, dependencies)) {
			ret = false;
			continue;
		}
		...
		if (ret && std::find(load_list.begin(), load_list.end(), &*it) == load_list.end()) {
			load_list.emplace_back(&*it);
		}
	}
