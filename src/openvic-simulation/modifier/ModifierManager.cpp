...
bool ModifierManager::_register_modifier_effect(
	CaseInsensitiveIdentifierRegistry<ModifierEffect, RegistryStorageInfoDeque>& registry,
...
NodeTools::key_value_callback_t ModifierManager::_expect_modifier_effect(
	CaseInsensitiveIdentifierRegistry<ModifierEffect, RegistryStorageInfoDeque> const& registry,
	ModifierValue& modifier_value
) const {
...
NodeTools::key_value_callback_t ModifierManager::_expect_modifier_effect_with_fallback(
	...
	return [this, &registry, &modifier_value, fallback]
		(const std::string_view key, const ast::NodeCPtr value) {
		if (dryad::node_has_kind<ast::ListValue>(value) && complex_modifiers.contains(key)) {
			return expect_dictionary([this, &modifier_value, &key]
				(const std::string_view inner_key, const ast::NodeCPtr inner_value) -> bool{
