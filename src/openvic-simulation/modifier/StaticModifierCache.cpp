...
bool StaticModifierCache::load_static_modifiers(ModifierManager& modifier_manager, const ast::NodeCPtr root) {
	...
	const auto set_static_country_modifier = [&key_map, &modifier_manager, &ret](Modifier& modifier) -> void {
		ret &= add_key_map_entry(
		    key_map,
		    modifier.get_identifier(),
		    ONE_EXACTLY,
		    expect_dictionary(modifier_manager.expect_base_country_modifier(modifier))
		);
	};

	const auto set_country_event_modifier =
	    [&key_map, &modifier_manager, &ret](const std::string_view identifier, const IconModifier::icon_t icon) -> void {
		ret &= add_key_map_entry(
		    key_map, identifier, ONE_EXACTLY, [&modifier_manager, identifier, icon](const ast::NodeCPtr value) -> bool {
			    ...
			    bool has_parsed_modifier = expect_dictionary(modifier_manager.expect_base_country_modifier(modifier_value))(
			        value
			    );
