	...
	public:
		...
		inline std::to_chars_result to_chars(char* first, char* last, size_t decimal_places = -1) const {
			...
			if (decimal_places == static_cast<size_t>(-1)) {
				result = std::to_chars(first, last, to_int64_t());
		...
		struct stack_string {
			static constexpr size_t array_length = std::numeric_limits<long double>::max_digits10;
