...
namespace OpenVic {
	...
	public:
		...
		constexpr std::from_chars_result from_chars(char const* begin, char const* end) {
			...
			while (dot_pointer && *dot_pointer != '.' && ++dot_pointer != end) {}
			...
			if (dot_pointer != begin) {
				...
				fixed_point_t adder;
				from_chars = from_chars_integer(begin, dot_pointer, adder);
				result += adder;
			}
			...
			value = (negative ? -result : result).value;
			return from_chars;
		}
		...
		static constexpr fixed_point_t parse(char const* str, char const* end, bool* successful = nullptr) {
			...
			std::from_chars_result result = value.from_chars(str, end);
			if (successful) {
