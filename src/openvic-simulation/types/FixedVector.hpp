...
namespace OpenVic::_detail {
	...
	public:
		...
		explicit FixedVector(const size_t capacity)
			: _max_size(capacity), _size(0), _allocator(), _data_start_ptr(allocator_traits::allocate(_allocator, capacity)) {}

		FixedVector(const size_t size, T const& value_for_all_indices)
			: _max_size(size), _size(size), _allocator(), _data_start_ptr(allocator_traits::allocate(_allocator, size)) {
		...
		FixedVector(const size_t size, GeneratorTemplateType&& generator)
			...
			for (size_t i = 0; i < size; ++i) {
				allocator_traits::construct(_allocator, begin() + i, generator(i));
			}
		}
		...
		FixedVector(const size_t size, GeneratorTemplateType&& generator)
			...
			for (size_t i = 0; i < size; ++i) {
				std::apply(
					[this, i](auto&&... args) {
						allocator_traits::construct(_allocator, begin() + i, std::forward<decltype(args)>(args)...);
					},
					generator(i)
				);
			}
		...
		iterator emplace_back(Args&&... args) {
			...
			allocator_traits::construct(_allocator, begin() + _size, std::forward<Args>(args)...);
			++_size;
			return end() - 1;
		}
