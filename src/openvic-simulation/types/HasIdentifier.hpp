...
template<typename T>
struct fmt::formatter<T, std::enable_if_t<std::is_base_of_v<OpenVic::HasIdentifier, T>, char>> : fmt::formatter<std::string_view> {
	auto format(const OpenVic::HasIdentifier& has_id, format_context& ctx) const {
		return formatter<std::string_view>::format(has_id.get_identifier(), ctx);
	}
};
...
namespace fmt {
	...
	struct formatter<validate_view<T>> : formatter<T> {
		auto format(const validate_view<T>& value, format_context& ctx) const {
			return value.ptr ? formatter<T>::format(*value.ptr, ctx) : formatter<std::string_view>::format("<NULL>", ctx);
		}
	};
}