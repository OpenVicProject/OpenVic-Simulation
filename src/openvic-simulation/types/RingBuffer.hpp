...
namespace OpenVic {
	...
	struct RingBuffer {
		...
		struct _iterator {
			...
			operator _iterator<typename allocator_traits::const_pointer>() const {
			...
			friend difference_type operator-(_iterator const& lhs, _iterator const& rhs) noexcept {
				return lhs._index > rhs._index ? lhs._index - rhs._index : -(rhs._index - lhs._index);
			}

			friend _iterator operator+(difference_type lhs, _iterator const& rhs) noexcept {
				return rhs + lhs;
			}

			friend auto operator<=>(_iterator const& lhs, _iterator const& rhs) noexcept {
				return std::tie(lhs._data, lhs._offset, lhs._index) <=> std::tie(rhs._data, rhs._offset, rhs._index);
			}

			friend bool operator==(_iterator const& lhs, _iterator const& rhs) noexcept {
				return &*lhs == &*rhs;
			}
		...
		std::vector<value_type> read_buffer(size_type read_size) {
			read_size = std::min(read_size, capacity());
			iterator last = begin();
			ranges::advance(last, read_size);
			std::vector<value_type> result { std::make_move_iterator(begin()), std::make_move_iterator(last) };
			erase(begin(), read_size);
			return result;
		}
