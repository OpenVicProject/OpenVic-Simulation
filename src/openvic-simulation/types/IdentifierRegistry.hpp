...
namespace OpenVic {
	...
	concept RegistryStorageInfo =
	    ...
	    requires(
	        typename StorageInfo<Item>::storage_type& items,
	        typename StorageInfo<Item>::storage_type const& const_items,
...
#define GETTERS(CONST) \
	...
	constexpr NodeTools::NodeCallback auto expect_item_index(NodeTools::Callback<index_t> auto callback, bool warn = false) \
	    CONST { \
...
#undef GETTERS
	...
	template<
	    ...
	    template<typename> typename StorageInfo = RegistryStorageInfoVector,
	    string_map_case Case = StringMapCaseSensitive>
	requires RegistryStorageInfo<
	             StorageInfo,
	             typename RegistryItemInfoValue<typename ValueInfo::internal_value_type>::item_type>
	...
	requires RegistryStorageInfo<
	             StorageInfo,
	             typename RegistryItemInfoInstance<typename ValueInfo::internal_value_type>::item_type>
	...
	requires RegistryStorageInfo<
	             StorageInfo,
	             typename RegistryItemInfoBaseInstance<typename ValueInfo::internal_value_type>::item_type>
...
#define IDENTIFIER_REGISTRY_INTERNAL_SHARED(singular, plural, registry, const_kw) \
	...
	constexpr decltype(registry)::external_value_type const_kw* get_##singular##_by_index(decltype(registry)::index_t index) \
	    const_kw { \
	...
	constexpr NodeTools::Callback<std::string_view> auto expect_##singular##_str( \
	    NodeTools::Callback<decltype(registry)::external_value_type const_kw&> auto callback, \
	...
	constexpr NodeTools::Callback<std::string_view> auto expect_##singular##_index_str( \
	    NodeTools::Callback<decltype(registry)::index_t> auto callback, bool allow_empty = false, bool warn = false \
	) const_kw { \
