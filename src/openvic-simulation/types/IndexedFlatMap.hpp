...
namespace OpenVic {
	...
	public:
		...
		requires(std::is_move_constructible_v<ValueType> || std::is_copy_constructible_v<ValueType>)
		         ...
		         && (!specialization_of<
		                std::remove_cvref_t<std::invoke_result_t<GeneratorTemplateType, ForwardedKeyType const&>>,
		                std::tuple>)
		         // ValueType(value_generator(key)) is valid constructor call
		         && std::constructible_from<
		                ValueType,
		                decltype(std::declval<GeneratorTemplateType>()(std::declval<ForwardedKeyType const&>()))>
		...
		IndexedFlatMap(keys_span_type new_keys, GeneratorTemplateType value_generator) :
		    keys(new_keys), min_index { type_safe::get(new_keys.front().index) },
		    max_index { type_safe::get(new_keys.back().index) }, values { create_empty, new_keys.size() } {
			static_assert(has_index<ForwardedKeyType>);
			if (!validate_new_keys(new_keys)) {
				return;
			}
		...
		template<typename OtherValueType>
		IndexedFlatMap<ForwardedKeyType, ValueType> operator+(
