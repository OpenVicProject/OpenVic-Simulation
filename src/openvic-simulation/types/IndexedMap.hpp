...
namespace OpenVic {
	...
	struct IndexedMap : public fixed_size_vector<Value> {
		...
		using keys_span_type = std::span<const key_type>;
	...
	private:
		std::span<const key_type> PROPERTY(keys, {});

	public:
		constexpr IndexedMap() = default;

		constexpr IndexedMap(std::span<const key_type> new_keys) : IndexedMap() {
			set_keys(new_keys);
		}
		...
		constexpr void set_keys(std::span<const key_type> new_keys) {
